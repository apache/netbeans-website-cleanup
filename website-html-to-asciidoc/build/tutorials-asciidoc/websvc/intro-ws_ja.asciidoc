...
:keywords: Apache NetBeans  intro-ws_ja
:description: Apache NetBeans  intro-ws_ja
:toc: left
:toc-title:
...
* *RESTベース。**RE*presentational *S*tate *T*ransferは、Webサービスの作成およびWebサービスとの通信のための新しい方法です。RESTでは、各リソースにURIがあり、HTTPヘッダーを操作することで制御されます。詳細は、link:#rest[RESTful Webサービス]を参照してください。
* *SOAP/WSDLベース。*従来のWebサービス・モデルでは、WebサービスのインタフェースはWSDLドキュメント(XMLの一種)を介して公開され、この中にURLが含まれています。その後のメッセージ交換はSOAPで行われますが、これも別のタイプのXMLドキュメントです。詳細は、link:#jaxws[SOAPベースのWebサービス]を参照してください。
