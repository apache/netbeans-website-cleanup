...
:keywords: Apache NetBeans  zillow_ru
:description: Apache NetBeans  zillow_ru
:toc: left
:toc-title:
...
* link:#get-api-key[Регистрация для получения ключа интерфейса API Zillow]
* link:#create-servlet[Создание сервлета]
* link:#drop-zillow-op[Добавление операции GetRegionChart Zillow к сервлету]
* link:#adding-api-key[Добавление ключа интерфейса API к проекту]
* link:#modify-servlet-code[Модификация сервлета для возвращения графического представления диаграммы]
* link:#running[Запуск сервлета региональной диаграммы Zillow]
...
image:new-zillow-servlet.png[title="Мастер создания сервлетов с ZillowRegionChartServlet"]
...
image:zillow-project-new.png[title="Вкладка 'Проекты' в среде IDE, на которой отображается новый сервлет"]
...
image:z-llow-getregionchart.png[title="Вкладка 'Службы' с развернутыми узлами 'Веб-службы' и Zillow"]
...
image:droppedcode.png[title="Код сервлета, в котором отображается вставленный блок try getRegionChart"]
...
image:zillow-project-complete.png[title="Полная структура проекта ZillowRegionChart"]

=== Добавление ключа интерфейса API к проекту

Чтобы проект мог получить доступ к службам Zillow, необходимо передать ему ключ интерфейса API, полученный при link:#get-api-key[регистрации]. Откройте файл `zillowrealestateserviceauthenticator.properties` и скопируйте в него ключа интерфейса API из письма, присланного Zillow в ответ на регистрацию.
...
image:zillow-getresponse-cc.png[title="В редакторе отображается завершение кода для методов resultObj"]
...
image:zillow-run-properties.png[title="Диалоговое окно свойств для проекта ZillowRegionChart"]

При успешном запуске проекта откроется окно браузера с региональной диаграммой.

image:zillow-chart.png[title="Диаграмма области Zillow"]
