...
:keywords: Apache NetBeans  zillow_zh_CN
:description: Apache NetBeans  zillow_zh_CN
:toc: left
:toc-title:
...
* link:#get-api-key[注册以接收 Zillow API 密钥]
* link:#create-servlet[创建 Servlet]
* link:#drop-zillow-op[将 Zillow GetRegionChart 操作添加到 Servlet]
* link:#adding-api-key[将 API 密钥添加到项目]
* link:#modify-servlet-code[修改 Servlet 以返回图表图]
* link:#running[运行 Zillow 区域图 Servlet]
...
image:new-zillow-servlet.png[title="带有 ZillowRegionChartServlet 的新建 Servlet 向导"]
...
image:zillow-project-new.png[title="显示新 servlet 的 IDE 中的 "Projects"（项目）标签"]
...
image:z-llow-getregionchart.png[title="展开了 Web 服务和 Zillow 节点的 "Services"（服务）标签"]
...
image:droppedcode.png[title="显示插入的 getRegionChart try 代码块的 servlet 代码"]
...
image:zillow-project-complete.png[title="ZillowRegionChart 项目的完整结构"]

=== 将 API 密钥添加到项目

为了使项目能够访问 Zillow 服务，必须传递link:#get-api-key[注册]时获取的 API 密钥。打开 `zillowrealestateserviceauthenticator.properties` 文件，复制并粘贴收到的 API 密钥（来自 Zillow 的注册确认电子邮件）。
...
image:zillow-getresponse-cc.png[title="显示 resultObj 方法的代码完成的编辑器"]
...
image:zillow-run-properties.png[title="ZillowRegionChart 项目的 "Properties"（属性）对话框"]

成功地运行此项目时，会打开一个显示区域图的浏览器窗口。

image:zillow-chart.png[title="Zillow 区域图"]
