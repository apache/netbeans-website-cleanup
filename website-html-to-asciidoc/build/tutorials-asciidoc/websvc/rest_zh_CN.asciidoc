...
:keywords: Apache NetBeans  rest_zh_CN
:description: Apache NetBeans  rest_zh_CN
:toc: left
:toc-title:
...
* link:#entity-classes[REST 风格的 Web 服务、持久性和实体类]
* link:#create-services-from-db[通过数据库创建 REST 风格的 Web 服务]
* link:#create-project[创建项目]
* link:#entities-and-services[生成实体类和 REST 风格的服务]
* link:#test-rest[测试 REST 风格的 Web 服务]
...
image:select-tables.png[title="“通过数据库新建实体类”向导的 "Database Tables"（数据库表）面板，其中显示已选定的 CUSTOMER 和 DISCOUNT_CODE 表"]
...
image:test-rest1.png[title="浏览器中 REST 风格的 Web 服务测试器登录页"]
