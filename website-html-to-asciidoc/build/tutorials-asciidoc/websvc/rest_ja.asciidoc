...
:keywords: Apache NetBeans  rest_ja
:description: Apache NetBeans  rest_ja
:toc: left
:toc-title:
...
* link:#entity-classes[RESTful Webサービス、永続性およびエンティティ・クラス]
* link:#create-services-from-db[データベースからのRESTful Webサービスの作成]
* link:#create-project[プロジェクトの作成]
* link:#entities-and-services[エンティティ・クラスとRESTfulサービスの生成]
* link:#test-rest[RESTful Webサービスのテスト]
...
image:select-tables.png[title="データベースからの新規エンティティ・クラス・ウィザードの「データベース表」パネルでCUSTOMERおよびDISCOUNT_CODE表が選択された状態"]
...
image:test-rest1.png[title="ブラウザでのRESTful Webサービス・テスターの開始ページ"]
