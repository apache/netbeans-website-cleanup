...
= js-toolkits-jquery_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  js-toolkits-jquery_zh_CN
:description: Apache NetBeans  js-toolkits-jquery_zh_CN
:toc: left
:toc-title:
...
* link:#settingup[设置 NetBeans 项目]
* link:#addingjquery[将 jQuery 库添加到项目中]
* link:#gettingacquainted[熟悉 jQuery]
* link:#addingaccordion[将 jQuery 可折叠面板窗口部件添加到项目中]
* link:#usingcss[将 jQuery 的默认主题用于样式增强]
* link:#summary[小结]
* link:#seealso[另请参见]
...
image:browser-test.png[title="查看浏览器窗口中的空 HTML 模板"]
...
1. 在 IDE 中，创建一个名为 `js` 的文件夹并将其添加到项目中。为此，请单击 IDE 工具栏中的 "New File"（新建文件）(image:new-file-btn.png[]) 按钮。（也可以按 Ctrl-N 组合键；在 Mac 上按 ⌘-N 组合键。）
...
|image:jquery-lib-php.png[title="将 jQuery 库直接粘贴到您的项目"] |image:jquery-lib-java.png[title="将 jQuery 库直接粘贴到您的项目"] 
|===
...
image:blue-test.png[title="单击文本时文本将变为蓝色"]
...
image:code-completion.png[title="按 Ctrl-空格键查看代码完成和 API 文档窗口"]
...
如果尚未执行此操作，请访问 link:http://jqueryui.com/download[http://jqueryui.com/download] 并下载可折叠面板导航窗口部件。请注意，在选择可折叠面板窗口部件时，会自动选择 UI 核心库和窗口部件工厂。还请注意，在下载页面中，默认情况下会选择 "`UI lightness`" 主题并将其包括在下载包中。在link:#usingcss[以下部分]中，会将此主题应用于联系人列表。
...
4. 现在，我们将向项目添加以前代码片段中涉及的 JPG 肖像。从link:#requiredSoftware[之前下载的项目资源]中检索 `pix` 目录，将整个目录复制到项目文件夹中，将其与 `index.html` 目录同级放置。稍后，NetBeans 将自动更新 "Projects"（项目）窗口，可以看到该项目中已经添加了一个新目录。
5. 切换到浏览器并刷新页面。
image:structured-list.png[title="结构化列表显示在浏览器中"]
...
7. 复制（Ctrl-C 组合键；在 Mac 上为 ⌘-C 组合键）这三个脚本，回到 IDE 中，将其粘贴到您之前在 `jqproject` 文件夹中link:#js[创建]的 `js` 文件夹。
...
image:accordion-list.png[title="可折叠面板窗口部件处理用户单击并生成可折叠面板效果"]
...
为此，请右键单击项目节点并选择 "New"（新建）> "Folder"（文件夹）。（如果文件夹未显示为选项，请单击 IDE 工具栏中的 "New File"（新建文件）(image:new-file-btn.png[]) 按钮，然后在新建文件向导中选择 "Other"（其他）> "Folder"（文件夹）。）将该文件夹命名为 `css` 并将其与 `index.html` 文件放在同一目录中。
...
|image:proj-win-php.png[title="项目包含 jQuery 默认主题"] |image:proj-win-java.png[title="项目包含 jQuery 默认主题"] 
|===
...
image:ui-lightness-theme.png[title="jQuery 默认主题增强可折叠面板窗口部件的外观"]
