...
:keywords: Apache NetBeans  oracle-cloud_zh_CN
:description: Apache NetBeans  oracle-cloud_zh_CN
:toc: left
:toc-title:
...
* link:#sdk-download[下载 Oracle Java Cloud Service SDK]
* link:#install-plugin[安装 Oracle Cloud 插件]
* link:#register-cloud[在 IDE 中注册 Oracle Cloud]
* link:#jobs-n-logs[查看作业和日志]
* link:#remote-server[Oracle Cloud Remote 服务器]
* link:#cloud-vs-local[在本地计算机上开发应用程序]
* link:#create-webapp[创建 Web 应用程序]
* link:#crud[使用 JSF 创建 CRUD 应用程序]
* link:#test-whitelist[测试白名单支持]
* link:#seeAlso[另请参见]
...
1. 在 "Projects"（项目）窗口中，右键单击link:#create-webapp[您创建的 Web 应用程序]，并选择 "New"（新建）> "Servlet"。此时将打开新建 Servlet 向导。
