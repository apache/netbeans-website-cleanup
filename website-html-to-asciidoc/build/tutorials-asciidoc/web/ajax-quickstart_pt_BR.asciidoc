...
* link:#servlet[Criando um Servlet]
...
* link:#httpMonitor[Utilizando o Monitor do Servidor HTTP]
...
image:java-name-location.png[title="Especifique o nome e a localização do seu projeto de aplicação"]
...
image:java-server-settings.png[title="Especifique o servidor no qual sua aplicação será implantada"]
...
image:java-proj-win.png[title="A janela Projetos exibe o projeto MyAjaxApp"]
...
* Método: GET
* Nome: autofillform
image:insert-form.png[title="Caixa de diálogo Inserir form"]
...
image:code-completion.png[title="Pressione Ctrl-Espaço no editor para chamar o suporte a autocompletar código e à documentação"]
...
image:javascript-options.png[title="Opções de Dicas de JavaScript na janela Opções"]
...
image:insert-code.png[title="Menu pop-up Inserir Código exibido no Editor de Código-fonte"]
...
image:new-override.png[title="A caixa de diálogo Substituir lista classes herdadas"]
...
image:import-hint.png[title="Dicas de importação são exibidas na margem esquerda do Editor de Código-fonte"]

O método `doGet()` precisa fazer parsing do URL da solicitação, recuperar dados do armazenamento de dados e preparar uma resposta em formato XML. Note que a declaração do método foi gerada quando você criou a classe. Para exibi-la, pode ser necessário expandir métodos HttpServlet clicando no ícone de expansão (image:expand-icon.png[]) na margem esquerda.
...
Nesse exemplo, o servlet gera um documento XML que contém todos os composers com nome ou sobrenome começando com os caracteres digitados pelo usuário. Este documento mapeia para os dados XML ilustrados no link:#flow-diagram[fluxograma] acima. Aqui está um exemplo de um documento XML que é retornado para o objeto `XMLHttpRequest`:
...
Você pode utilizar o link:#httpMonitor[Monitor HTTP] do IDE para exibir os dados XML retornados ao concluir a aplicação.
...
* link:#callback[Adicionando a Funcionalidade de Retorno de Chamada]
* link:#htmldom[Atualizando o HTML DOM]
* link:#displayresults[Exibindo os Resultados]
* link:#stylesheet[Anexando uma Folha de Estilo]
...
image:css-styles-window.png[title="Janela Estilos de CSS exibindo as propriedades de regra h1"]

Você pode usar a janela Estilos de CSS para exibir rapidamente as propriedades e editar as regras de estilo. Quando você seleciona uma regra no painel superior da janela Estilo de CSS, pode ver as propriedades da regra no painel inferior. Você pode adicionar uma Regra de CSS à folha de estilos clicando no ícone Editar Regras de CSS (image:newcssrule.png[title="Ícone Nova Propriedade de CSS"]) na barra de ferramentas do painel superior. Você pode modificar as regras no painel inferior, editando a folha de propriedades, e adicionar propriedades clicando no ícone Adicionar Propriedade (image:newcssproperty.png[title="Ícone Nova Propriedade de CSS"]) na barra de ferramentas do painel inferior.
...
image:css-completion.png[title="Recurso autocompletar código no editor"]
...
image:http-monitor.png[title="O monitor do Servidor HTTP é exibido no IDE"]
...
image:xml-data.png[title="O monitor do Servidor HTTP é exibido no IDE"]
