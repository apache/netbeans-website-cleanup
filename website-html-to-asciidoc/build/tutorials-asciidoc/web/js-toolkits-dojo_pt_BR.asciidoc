...
= js-toolkits-dojo_pt_BR
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  js-toolkits-dojo_pt_BR
:description: Apache NetBeans  js-toolkits-dojo_pt_BR
:toc: left
:toc-title:
...
* link:#addLib[Abrindo o Projeto de Amostra]
* link:#linkFromFile[Vinculando os Recursos do Kit de ferramentas de um Arquivo do Projeto]
* link:#addWidget[Adicionando e Configurando o Widget Árvore do Dojo]
* link:#addJSON[Adicionando Códigos-Fonte de Conversão JSON de Terceiros como um Arquivo JAR ao Projeto]
* link:#prepareServlet[Preparando um Servlet para Iniciar uma Resposta JSON]
...
image:dojo-tree-complete.png[title="Exercício concluído exibido em um browser"]
...
2. Clique no botão Abrir Projeto ( image:open-project-btn.png[] ) na barra de ferramentas do IDE para abrir a caixa de diálogo Abrir Projeto.
...
image:proj-win-ref-problems.png[title="Os problemas de referência do projeto Project são indicados por um texto vermelho e um badge de erro"]

O problema de referência existe porque as classes Java usadas no projeto (`Tribe` e `TribeDataManager`) referenciam classes encontradas no arquivo JSON JAR, que você adicionará mais tarde na seção link:#addJSON[Adicionando Arquivos JSON JAR ao Projeto].
...
image:proj-win-dojo-resources.png[title="As bibliotecas Dojo e Dijit são adicionadas à pasta 'recursos' do Projeto"]

Selecione o GlassFish Server na caixa de diálogo Resolver Referências. Clique em OK.

image:dojo-add-server.png[title="As bibliotecas Dojo e Dijit são adicionadas à pasta 'recursos' do Projeto"]
...
----
* `link:http://docs.dojocampus.org/dojo/data/ItemFileReadStore[dojo.data.ItemFileReadStore]`: lê o conteúdo com estrutura JSON de um ponto final HTTP (em link:#prepareServlet[Preparando um Servlet para Iniciar uma Resposta JSON], você implementará um servlet com essa finalidade) e armazena todos os itens residentes na memória para um acesso rápido e simples.
* `link:http://docs.dojocampus.org/dijit/Tree[dijit.Tree]`: o widget Árvore que fornece uma view dos dados JSON recuperados do `ItemFileReadStore`.
...
3. Clique no botão Novo Projeto (image:new-project-btn.png[]) na barra de ferramentas para abrir o assistente do Novo Projeto.
...
image:proj-win-json-sources.png[title="Agora, os códigos-fonte estão contidos no novo projeto 'json'"]
...
image:files-win-compiled-classes.png[title="Os códigos-fonte criados podem ser exibidos em uma pasta 'build' do projeto"]
...
image:libraries-json-jar.png[title="O arquivo jar é referenciado pelo Projeto"]
...
image:dojo-navigator.png[title="Utilizar o Navigator para exibir os campos e propriedades da classe"]
...
image:javadoc-window.png[title="O Javadoc para TribeDataManager fornece um exemplo de dados JSON"]
...
image:debugger-breakpoint.png[title="Usar o Depurador Java para a etapa por meio de código"]
2. Selecione o projeto `DojoTreeSample` na janela Projetos.
3. Execute o depurador (clique no botão Depurar Projeto (image:debug-btn.png[] ) da barra de ferramentas).
4. Use os botões Fazer Step Into (image:step-into-btn.png[]) e Fazer Step Over (image:step-over-btn.png[]) da barra de ferramentas.
...
link:#TribeServlet[Lembre-se de que você especificou '`TribeServlet`' como o valor para a propriedade] `url` ao adicionar o `ItemFileReadStore` à página Web. Esse é o destino no servidor que é encarregado da preparação e retorno dos dados JSON ao cliente. Agora, vamos criar esse servlet.
...
image:new-servlet-wizard.png[title="Utilizar o assistente de Novo Servlet para criar servlets"]
...
import dojo.org.json.JSONObject;
----
7. Para executar o projeto, selecione o nó do projeto `DojoTreeSample` na janela Projetos e, em seguida, clique no botão Executar Projeto (image:run-project-btn.png[]) na barra de ferramentas do IDE.

O browser será aberto e exibirá uma página de boas-vindas (`dojoDemo.html`), e você poderá ver que o widget Árvore do Dojo exibirá dados da ArrayList corretamente, como na link:#final[captura de tela acima].
