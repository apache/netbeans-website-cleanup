...
:keywords: Apache NetBeans  hibernate-webapp_ja
:description: Apache NetBeans  hibernate-webapp_ja
:toc: left
:toc-title:
...
* link:#03[Hibernate構成ファイルの変更]
* link:#04[`HibernateUtil.java`ヘルパー・ファイルの作成]
* link:#05[Hibernateのマッピング・ファイルおよびJavaクラスの生成]
* link:#06[`FilmHelper.java`ヘルパー・クラスの作成]
* link:#07[JSF管理対象Beanの作成]
* link:#08[Webページの作成]
* link:#09[プロジェクトの実行]
* link:#10[ソリューション・プロジェクトのダウンロード]
* link:#11[トラブルシューティング]
...
image:hib-newwebapp.png[title="プロジェクトへのHibernateサポートの追加を示す新規プロジェクト・ウィザードの「フレームワーク」パネル"]
...
image:hib-libraries.png[title="Hibernateライブラリが表示された「プロジェクト」ウィンドウのスクリーンショット"]
...
image:add-property-sessioncontext.png[title="hibernate.current_session_context_classプロパティの設定値が表示された「Hibernateのプロパティの追加」ダイアログ・ボックス"]
...
image:add-property-factoryclass.png[title="hibernate.query.factory_classプロパティの設定値が表示された「Hibernateのプロパティの追加」ダイアログ・ボックス"]
...
image:hibernate-util-wizard.png[title="HibernateUtilウィザードのスクリーンショット"]
...
image:hibernate-reveng-wizard.png[title="新規Hibernateリバース・エンジニアリング・ウィザード"]
...
image:hibernate-pojo-wizard2.png[title="「Hibernateのマッピング・ファイルとPOJOの生成」ウィザード"]
...
image:hib-projectswindow.png[title="生成されたPOJOを示す「プロジェクト」ウィンドウ"]
...
4. 接続をテストするには、エディタで次のように入力し、ツールバーの「HQL問合せの実行」ボタン(image:run_hql_query_16.png[title="「HQL問合せの実行」ボタン"])をクリックします。
[source,java]
----
...
image:hibernate-hqleditor1.png[title="「Hibernateのマッピング・ファイルとPOJOの生成」ウィザード"]
...
image:hib-newmanagedbean.png[title="新規JSF管理対象Beanウィザード"]
...
image:hib-faceletsclient.png[title="新規ファイル・ウィザードの「Faceletsテンプレート・クライアント」ファイル・タイプ"]
...
image:hib-browsetemplate.png[title="「ファイルを参照」ダイアログでのテンプレートの選択"]
...
image:hib-browser1.png[title="indexページに映画のリストが表示されたスクリーンショット"]
