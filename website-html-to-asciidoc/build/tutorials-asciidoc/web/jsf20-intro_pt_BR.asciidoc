...
:keywords: Apache NetBeans  jsf20-intro_pt_BR
:description: Apache NetBeans  jsf20-intro_pt_BR
:toc: left
:toc-title:
...
* link:#support[Adicionando Suporte JSF 2.x a uma Aplicação Web]
* link:#managedBean[Criando um Bean Gerenciado]
* link:#usingManagedBean[Utilizando o Assistente de Bean Gerenciado]
* link:#creatingConstructor[Criando um Construtor]
* link:#addingProperties[Adicionando Propriedades]
* link:#wire[Conectando o Bean Gerenciado a Páginas]
* link:#index[index.xhtml]
* link:#response[response.xhtml]
* link:#template[Aplicando um Modelo de Facelets]
* link:#templateFile[Criando o Arquivo de Modelo de Facelets]
* link:#templateClient[Criando Arquivos Clientes de Modelo]
...
Comece abrindo o link:#requiredSoftware[`jsfDemo` projeto da aplicação Web] no IDE. Depois de abrir o projeto no IDE, você pode adicionar suporte ao framework utilizando a janela Propriedades do projeto.
...
1. Clique no botão ( image:open-project-btn.png[] ) para Abrir o Projeto na barra de ferramentas principal do IDE ou pressione Ctrl-Shift-O (⌘-Shift-O no Mac).
...
3. Execute o projeto para ver como ele é em um browser. Clique com o botão direito do mouse no nó `jsfDemo` do projeto, na janela Projetos e selecione Executar ou clique no botão ( image:run-project-btn.png[] ) Executar Projeto na barra de ferramentas principal. O projeto é encapsulado e implantado no GlassFish Server e o browser é aberto para exibir a página de boas-vindas (`index.xhtml`).
image:run-project.png[title="Executar o projeto para exibi-lo em um browser"]
4. Clique no botão Submeter. A página de resposta (`response.xhtml`) é exibida desta forma:
image:response.png[title="A página de boas-vindas e de resposta, atualmente, são páginas estáticas"]
...
image:libraries-tab.png[title="Especificar o acesso a bibliotecas do JSF 2.x"]
...
image:jsf-configuration.png[title="Especificar idioma preferencial e opções de servlet do Faces"]
...
image:jsf-components.png[title="Especificar idioma preferencial e opções de servlet do Faces"]
...
* *Nome:* UserNumberBean
* *Escopo:* Session
image:managed-bean.png[title="Utilizar o Ben Gerenciado pelo JSF para criar um novo Bean gerenciado"]
...
2. Corrigir importações. Para isso, clique no indicador de dica ( image:hint-icon.png[] ) exibido na margem esquerda do editor, em seguida, selecione a opção para importar `java.util.Random` para a classe.
3. Execute o projeto novamente (clique no botão ( image:run-project-btn.png[] ) Executar Projeto ou pressione F6; fn-F6 no Mac). Quando você executa seu projeto, o arquivo de log do servidor é aberto automaticamente na janela de Saída.
image:output1.png[title="O arquivo de log do servidor é aberto automaticamente na janela de Saída"]
...
----
Utilize o indicador de dica ( image:hint-icon.png[] ) para importar `java.io.Serializable` para a classe.
...
image:getter-setter.png[title="Utilizar o IDE para gerar métodos de acesso para as propriedades"]
3. Selecione a opção `userNumber` : `Integer`. Clique em Gerar.
image:generate-getters-setters.png[title="Utilizar o IDE para gerar métodos de acesso para as propriedades"]
...
image:comment-out.png[title="Realce o código, em seguida, pressione Ctrl-/ para comentá-lo"]
...
image:comment.png[title="Realce o código comentado e, em seguida, pressione Ctrl-/ para eliminar o comentário"]
...
image:undeclared-component.png[title="O editor fornecerá mensagens de erro para os componentes não declarados"]
...
As etapas de 7 a 12 a seguir são opcionais. Se quiser construir o projeto rapidamente, passe para link:#response[`response.xhtml`].
...
image:set-breakpoint.png[title="Clique no margem esquerda do editor para definir pontos de interrupção"]
8. Clique no botão Depurar Projeto (image:breakpoint-btn.png[]) na barra de ferramentas principal do IDE. A sessão de depuração é iniciada, e a página de boas-vindas do projeto é aberta no browser.
...
image:debugger-suspended.png[title="O depurador é suspenso de acordo com pontos de interrupção"]
...
image:variables-window.png[title="Monitorar valores das variáveis utilizando a janela Variáveis do Depurador"]
...
12. Na barra de ferramentas do Depurador, clique no botão Fazer Step Into (image:step-into-btn.png[]). O depurador executará a linha na qual ele está atualmente suspenso. A janela Variáveis se será atualiza, indicando as alterações na execução.
image:variables-window2.png[title="A janela de variáveis é atualizada ao percorrer o código"]
...
image:code-completion2.png[title="Pressione Ctrl-Espaço para chamar uma janela pop-up com as sugestões da funcionalidade autocompletar código e a documentação"]
...
----
7. Execute o projeto (clique no botão ( image:run-project-btn.png[] ) Executar Projeto ou pressione F6; fn-F6 no Mac). Quando a página de boas-vindas for exibida no browser, informe um número e clique em `submeter`. Você verá a página de resposta com uma aparência semelhante à seguinte (contanto que você não adivinhe o número correto):
image:response2.png[title="Exibir o status atual do projeto em um browser"]

Há duas coisas erradas no status atual da página de resposta:

1. As tags html `<p>` são exibidas na mensagem de resposta.
2. O botão Voltar não está sendo exibido no lugar correto. (Compare-a à link:#originalVersion[versão original].)
...
image:escape-false.png[title="Pressione Ctrl-Espaço para exibir possíveis valores do atributo e a documentação"]
...
10. Execute o projeto novamente (clique no botão ( image:run-project-btn.png[] ) Executar Projeto ou pressione F6; fn-F6 no Mac). Insira um número na página de boas-vindas e clique em Submeter. A página de resposta agora exibe a mensagem de resposta sem as tags `<p>` e o botão Voltar está no lugar correto.
image:response3.png[title="Exibir o status atual do projeto em um browser"]
...
image:yay.png[title="A resposta correta será exibida ao inserir o número correspondente"]
14. Clique novamente no botão Voltar. Observe que o número inserido anteriormente não será mais exibido no campo de texto. Lembre-se de que o método `getResponse()` do `UserNumberBean` link:#getResponse[invalida a sessão atual do usuário] quando o número correto é descoberto.
...
image:layout-style.png[title="O assistente de Modelo de Facelets permite selecionar entre estilos de layout comuns"]
...
----
5. Examine novamente as páginas de link:#staticPage[boas-vindas] e de link:#responsePage[resposta]. O único conteúdo que é alterado nas duas páginas é o título e o texto contido no quadro cinza. O modelo, portanto, pode fornecer todo o conteúdo restante.
...
image:facelets-template.png[title="Exibir o modelo de Facelets em um browser"]
...
image:template-client.png[title="Assistente de Cliente de Modelo de Facelets"]
...
image:welcome-files.png[title="Alterar a entrada de Arquivos de Boas-Vindas no descritor de implantação"]
3. Execute o projeto para ver como ele é em um browser. Pressione F6 (fn-F6 no Mac) ou clique no botão (image:run-project-btn.png[]) Executar Projeto na barra de ferramentas principal. O projeto será implantado no GlassFish Server e aberto em um browser.
