...
:keywords: Apache NetBeans  oracle-cloud_ja
:description: Apache NetBeans  oracle-cloud_ja
:toc: left
:toc-title:
...
* link:#sdk-download[Oracle Java Cloud Service SDKのダウンロード]
* link:#install-plugin[Oracle Cloudプラグインのインストール]
* link:#register-cloud[IDEへのOracle Cloudの登録]
* link:#jobs-n-logs[ジョブおよびログの表示]
* link:#remote-server[Oracle Cloud Remoteサーバー]
* link:#cloud-vs-local[ローカル・マシンでのアプリケーションの開発]
* link:#create-webapp[Webアプリケーションの作成]
* link:#crud[JSFでのCRUDアプリケーションの作成]
* link:#test-whitelist[ホワイトリスト・サポートのテスト]
* link:#seeAlso[関連項目]
...
1. 「プロジェクト」ウィンドウで、link:#create-webapp[作成したWebアプリケーション]を右クリックし、「新規」→「サーブレット」を選択します。新規サーブレット・ウィザードが開きます。
