...
:keywords: Apache NetBeans  js-toolkits-dojo_ja
:description: Apache NetBeans  js-toolkits-dojo_ja
:toc: left
:toc-title:
...
* link:#addLib[サンプル・プロジェクトを開く]
* link:#linkFromFile[プロジェクト・ファイルからツールキット・リソースへのリンク]
* link:#addWidget[Dojoツリー・ウィジェットの追加と構成]
* link:#addJSON[サードパーティJSON変換ソースをJARファイルとしてプロジェクトに追加]
* link:#prepareServlet[JSONレスポンスを開始するサーブレットの準備]
* link:#seeAlso[関連項目]
...
image:dojo-tree-complete.png[title="ブラウザに表示された完成した課題"]
...
2. IDEツールバーの「プロジェクトを開く」ボタン(image:open-project-btn.png[])をクリックして、「プロジェクトを開く」ダイアログ・ボックスを開きます。
...
image:proj-win-ref-problems.png[title="プロジェクトの参照の問題を示す赤いテキストとエラー・バッジ"]

プロジェクトで使用されているJavaクラス(`Tribe`および`TribeDataManager`)はJSON JARファイルに見つかったクラスを参照していますが、これは後でlink:#addJSON[「サードパーティJSON変換ソースをJARファイルとしてプロジェクトに追加」]の項で追加するため、参照の問題が存在します。
...
image:proj-win-dojo-resources.png[title="プロジェクトの「resources」フォルダに追加されたDojoライブラリとDijitライブラリ"]

「参照の解決」ダイアログ・ボックスで「GlassFish Server」を選択します。「OK」をクリックします。

image:dojo-add-server.png[title="プロジェクトの「resources」フォルダに追加されたDojoライブラリとDijitライブラリ"]
...
----
* `link:http://docs.dojocampus.org/dojo/data/ItemFileReadStore[dojo.data.ItemFileReadStore]`: JSON構造化コンテンツをHTTPエンドポイントから読み取り(link:#prepareServlet[JSONレスポンスを開始するサーブレットの準備]で、この目的に使用するサーブレットを実装します)、すべての項目をメモリー内に保存して単純にすばやくアクセスできるようにします。
* `link:http://docs.dojocampus.org/dijit/Tree[dijit.Tree]`: `ItemFileReadStore`から取得されたJSONデータを表示するツリー・ウィジェットです。
...
3. ツールバーの「新規プロジェクト」ボタン(image:new-project-btn.png[])をクリックして新規プロジェクト・ウィザードを開きます。
...
image:proj-win-json-sources.png[title="新しい「json」プロジェクトに含まれるようになったソース"]
...
image:files-win-compiled-classes.png[title="プロジェクトの「build」フォルダに表示されたコンパイル済ソース"]
...
image:libraries-json-jar.png[title="プロジェクトにより参照されるJARファイル"]
...
image:dojo-navigator.png[title="ナビゲータを使用した、クラスのフィールドおよびプロパティの表示"]
...
image:javadoc-window.png[title="JSONデータの例を示すTribeDataManagerのJavadoc"]
...
image:debugger-breakpoint.png[title="Javaデバッガを使用したコードのステップ実行"]
2. 「プロジェクト」ウィンドウで「`DojoTreeSample`」プロジェクトを選択します。
3. デバッガを実行します(ツールバーで「プロジェクトをデバッグ」(image:debug-btn.png[])ボタンをクリックします)。
4. ツールバーの「ステップ・イン」(image:step-into-btn.png[])ボタンと「ステップ・オーバー」(image:step-over-btn.png[])ボタンを使用します。
...
Webページに`ItemFileReadStore`を追加したとき、link:#TribeServlet[`url`プロパティの値として「`TribeServlet`」を指定]しました。クライアントに対してJSONデータを準備して返す作業を担当するサーバー側では、これは送信先です。このサーブレットを作成しましょう。
...
image:new-servlet-wizard.png[title="新規サーブレット・ウィザードを使用したサーブレットの作成"]
...
import dojo.org.json.JSONObject;
----
7. プロジェクトを実行するには、「プロジェクト」ウィンドウで`DojoTreeSample`プロジェクト・ノードを選択し、IDEのツールバーの「プロジェクトの実行」(image:run-project-btn.png[])ボタンをクリックします。

ブラウザが開いて開始画面(`dojoDemo.html`)が表示され、link:#final[上のスクリーンショット]のように、Dojo TreeウィジェットでArrayListのデータが正しく表示されます。
