...
:keywords: Apache NetBeans  applets_ja
:description: Apache NetBeans  applets_ja
:toc: left
:toc-title:
...
* link:#create[アプレットの作成またはインポート]
* link:#runanddebug[アプレットの実行]
* link:#embed[アプレットのWebアプリケーションへの埋込み]
...
4. 「終了」をクリックします。アプレット・ソース・ファイルをインポートした場合は、link:#runanddebug[それを実行]します。
