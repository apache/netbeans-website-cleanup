...
:keywords: Apache NetBeans  ajax-quickstart
:description: Apache NetBeans  ajax-quickstart
:toc: left
:toc-title:
...
* link:#client1[Programming the Client-Side: Part 1]
* link:#html[Using the HTML Editor]
* link:#javascript[Using the JavaScript Editor]
* link:#serverside[Programming the Server-Side]
* link:#data[Creating the Data Store]
* link:#servlet[Creating a Servlet]
* link:#client2[Programming the Client-Side: Part 2]
* link:#callback[Adding Callback Functionality]
* link:#htmldom[Updating the HTML DOM]
* link:#stylesheet[Attaching a Stylesheet]
* link:#run[Running the Application]
* link:#httpMonitor[Using the HTTP Server Monitor]
...
The `onkeyup` attribute that you typed in above points to a JavaScript function named `doCompletion()`. This function is called each time a key is pressed in the form text field, and maps to the JavaScript call depicted in the Ajax link:#flow-diagram[flow diagram] above.
...
and a `callback()` function link:#callback[must later be defined]. The HTTP interaction begins when `XMLHttpRequest.send()` is called. This action maps to the HTTP request that is sent to the web server in the above link:#flow-diagram[flow diagram].
...
You can use the IDE's link:#httpMonitor[HTTP Monitor] to view the returned XML data once you complete the application.
...
* link:#callback[Adding Callback Functionality]
* link:#htmldom[Updating the HTML DOM]
* link:#displayresults[Displaying the Results]
* link:#stylesheet[Attaching a Stylesheet]
...
Notice that the `parseMessages()` function is called only when the `XMLHttpRequest.readyState` is "4" and the `status` -- the HTTP status code definition of the request -- is "200", signifying a success. You will define `parseMessages()` next in link:#htmldom[Updating the HTML DOM].
...
The dynamic update to the `complete-table` element represents the final step of the process flow of communication that takes place during communication using Ajax. This update maps to the HTML &amp; CSS data being sent to the presentation in the link:#flow-diagram[flow diagram] above.
...
You can use the CSS Styles window to quickly view the properties and edit style rules. When you select a rule in the upper pane of the CSS Style window you can see the properties of the rule in the lower pane. You can add a CSS Rules to the stylesheet by clicking the Edit CSS Rules icon (image:newcssrule.png[title="New CSS Property icon"]) in the toolbar of the upper pane. You can modify rules in the lower pane by editing the property sheet and add properties by clicking the Add Property icon (image:newcssproperty.png[title="New CSS Property icon"]) in the toolbar of the lower pane.
