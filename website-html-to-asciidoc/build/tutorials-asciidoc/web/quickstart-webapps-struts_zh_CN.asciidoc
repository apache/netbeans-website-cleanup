...
= quickstart-webapps-struts_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  quickstart-webapps-struts_zh_CN
:description: Apache NetBeans  quickstart-webapps-struts_zh_CN
:toc: left
:toc-title:
...
* link:#set[设置 Struts 应用程序]
* link:#jsp[创建 JSP 页]
* link:#login[创建登录页]
* link:#success[创建成功页]
* link:#actionForm[创建 `ActionForm` Bean]
* link:#actionClass[创建 `Action` 类]
* link:#validate[实现验证]
* link:#beanData[访问 Bean 数据并准备转发条件]
* link:#errorMsg[设置错误消息]
* link:#forward[将 `forward` 条目添加到 `struts-config.xml` 中]
* link:#configure[配置和运行应用程序]
* link:#welcome[设置欢迎页]
* link:#style[附加样式表]
* link:#run[运行应用程序]
* link:#seeAlso[另请参见]
...
image:new-project-wizard.png[title="Struts 选项显示在新建 Web 应用程序向导的 "Frameworks"（框架）面板中"]
...
image:proj-window-init.png[title=""Projects"（项目）窗口将显示 MyStrutsApp 项目"]
...
image:code-completion.png[title="为 Struts 标记提供代码完成和 Javadoc"]
...
5. 在 IDE 右侧区域的组件面板（"Window"（窗口）> "Palette"（组件面板））中，将 "Table"（表）项从 "HTML" 类别拖动至 `<html:submit value="Login" />` 行上方的某个位置。将显示 "Insert Table"（插入表格）对话框。将行设置为 `3`，列设置为 `2`，其他所有设置都保留为 `0`。在本教程中，稍后您将link:#style[附加样式表]来影响表的显示。
image:insert-table.png[title=""Palette"（组件面板）为易于使用的代码模板提供对话框"]
...
image:navigation-support.png[title="在 struts-config.xml 中提供导航支持"]
...
image:create-accessors.png[title="在源代码编辑器中按 Ctrl-I 时显示 "Insert Code"（插入代码）菜单"]
...
image:new-struts-action.png[title="新建 Struts 操作向导"]
...
如果用户同时提供了 `name` 和 `email` 字段的条目，并且输入的电子邮件地址中包含“@”符号，则 `execute` 方法将使用以上逻辑将请求转发到 `success` 视图。否则，将转发 `failure` 视图。您可以将 `failure` 视图设置为指回窗体页面，允许用户再次输入格式正确的数据，详细信息请参见下面的link:#forward[将 `forward` 条目添加到 `struts-config.xml`]。
...
----
2. 为 `error` 添加 getter 方法和 setter 方法，link:#accessors[如上所述]。
...
image:add-forward.png[title="右键单击并选择 "Struts" > "Add Forward"（添加转发）"]
...
image:add-forward-dialog.png[title=""Add Forward"（添加转发）对话框在 struts-config.xml 中创建一个转发条目"]
...
image:welcome-files.png[title="应用程序的部署描述符的图形编辑器"]
...
image:login-form.png[title="窗体包含将验证失败的数据"]

单击 "Login" 之后，登录窗体页会重新显示，其中包含一条错误消息：
image:login-form-error.png[title="窗体重新显示，但具有错误消息"]

尝试输入应该通过验证的数据。单击 "Login" 之后，您将会看到成功页：
image:success-page.png[title="成功页显示，并显示输入数据"]
