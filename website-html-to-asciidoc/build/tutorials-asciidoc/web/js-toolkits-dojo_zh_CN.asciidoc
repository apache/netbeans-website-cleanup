...
= js-toolkits-dojo_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  js-toolkits-dojo_zh_CN
:description: Apache NetBeans  js-toolkits-dojo_zh_CN
:toc: left
:toc-title:
...
* link:#addLib[打开样例项目]
* link:#linkFromFile[从项目文件链接到工具包资源]
* link:#addWidget[添加和配置 Dojo 树窗口部件]
* link:#addJSON[将第三方 JSON 转换源代码作为 JAR 文件添加到项目中]
* link:#prepareServlet[准备 Servlet 以启动 JSON 响应]
* link:#seeAlso[另请参见]
...
* 完成的项目如下所示：
image:dojo-tree-complete.png[title="在浏览器中查看的已完成练习"]
...
2. 单击 IDE 工具栏中的 "Open Project"（打开项目）按钮 (image:open-project-btn.png[]) 按钮以打开 "Open Project"（打开项目）对话框。
...
image:proj-win-ref-problems.png[title="项目引用问题由红色文本和错误标记指示"]

存在此引用问题的原因在于：项目（`Tribe` 和 `TribeDataManager`）中使用的 Java 类引用 JSON JAR 文件中的类，稍后您将在link:#addJSON[为项目添加 JSON JAR 文件]部分中添加该文件。
...
image:proj-win-dojo-resources.png[title="Dojo 和 Dijit 库已添加到项目的 &quot;resources&quot; 文件夹中"]

在 "Resolve References"（解决引用）对话框中选择 "GlassFish Server"。单击 "OK"（确定）。

image:dojo-add-server.png[title="Dojo 和 Dijit 库已添加到项目的 &quot;resources&quot; 文件夹中"]
...
----
* `link:http://docs.dojocampus.org/dojo/data/ItemFileReadStore[dojo.data.ItemFileReadStore]`：从 HTTP 端点读取 JSON 结构化内容（在link:#prepareServlet[准备 Servlet 以启动 JSON 响应]中，将为此用途实现 servlet。）并将所有项都存储在内存中，以便简单、快速地进行访问。
* `link:http://docs.dojocampus.org/dijit/Tree[dijit.Tree]`：树窗口部件，它提供了从 `ItemFileReadStore` 中检索的 JSON 数据的视图。
...
3. 单击工具栏中的 "New Project"（新建项目）按钮 (image:new-project-btn.png[]) 以便打开 "New Project"（新建项目）向导。
...
image:proj-win-json-sources.png[title="源文件现已包含在新的 &quot;json&quot; 项目中"]
...
image:files-win-compiled-classes.png[title="可在项目的 &quot;build&quot; 文件夹中查看已编译源代码"]
...
image:libraries-json-jar.png[title="JAR 文件由项目引用"]
...
----
每个 `Tribe` 对象捕获三个信息点：_tribe_、_category_ 和 _region_。此练习中的数据摘自维基百科中有关 link:http://en.wikipedia.org/wiki/Native_Americans_in_the_United_States#Ethno-linguistic_classification[Native Americans in the United States] 的条目。您可以确定，在一个 category 中可以对多个 _tribe_ _分类_，一个大的 _region_ 中可以包含许多 category。
...
image:dojo-navigator.png[title="使用导航器查看类字段和属性"]
...
image:javadoc-window.png[title="TribeDataManager 的 Javadoc 提供 JSON 数据的示例"]
...
image:debugger-breakpoint.png[title="使用 Java 调试器逐步执行代码"]
2. 在 "Projects"（项目）窗口中选择 `DojoTreeSample` 项目。
3. 运行调试器（在工具栏中单击 "Debug Project"（调试项目）按钮 (image:debug-btn.png[])）。
4. 使用工具栏中的 "Step Into"（步入）(image:step-into-btn.png[]) 和 "Step Over"（步过）(image:step-over-btn.png[]) 按钮。
...
回想一下，您在向 Web 页中添加 `ItemFileReadStore` 时，link:#TribeServlet[已将 "`TribeServlet`" 指定为 `url` 属性的值]。这是任务为准备 JSON 数据并将其返回客户端的服务器端中的目标。现在，我们要创建此 Servlet。
...
image:new-servlet-wizard.png[title="使用新建 Servlet 向导创建 Servlet"]
...
import dojo.org.json.JSONObject;
----
7. 要运行项目，请在 "Projects"（项目）窗口中选择 `DojoTreeSample` 项目节点，然后单击 IDE 工具栏中的 "Run Project"（运行项目）(image:run-project-btn.png[]) 按钮。

浏览器将打开以显示欢迎页 (`dojoDemo.html`)，您可以看到 Dojo 数窗口部件正在正确显示 ArrayList 中的数据，如link:#final[上面的屏幕快照]所示。
