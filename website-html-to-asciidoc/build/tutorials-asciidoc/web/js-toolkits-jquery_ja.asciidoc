...
= js-toolkits-jquery_ja
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  js-toolkits-jquery_ja
:description: Apache NetBeans  js-toolkits-jquery_ja
:toc: left
:toc-title:
...
* link:#settingup[NetBeansプロジェクトの設定]
* link:#addingjquery[プロジェクトへのjQueryライブラリの追加]
* link:#gettingacquainted[jQueryに習熟する]
* link:#addingaccordion[プロジェクトへのjQueryアコーディオン・ウィジェットの追加]
* link:#usingcss[jQueryのデフォルト・テーマを使用したスタイル拡張]
* link:#summary[サマリー]
* link:#seealso[関連項目]
...
image:browser-test.png[title="ブラウザ・ウィンドウでの空のHTMLテンプレートの表示"]
...
1. IDEで、`js`という名前のフォルダを作成し、プロジェクトに追加します。そのためには、IDEのツールバーにある「新規ファイル」(image:new-file-btn.png[])ボタンをクリックします。または、[Ctrl]-[N] (Macの場合は[⌘]-[N])を押します。
...
|image:jquery-lib-php.png[title="プロジェクトへのjQueryライブラリの直接貼付け"] |image:jquery-lib-java.png[title="プロジェクトへのjQueryライブラリの直接貼付け"] 
|===
...
image:blue-test.png[title="テキストをクリックしたときの青への変化"]
...
image:code-completion.png[title="[Ctrl]-[Space]の押下による、コード補完ウィンドウとAPIドキュメント・ウィンドウの表示"]
...
まだ実行していない場合は、link:http://jqueryui.com/download[http://jqueryui.com/download]にアクセスし、アコーディオン・ナビゲーション・ウィジェットをダウンロードします。アコーディオン・ウィジェットを選択すると、UIコア・ライブラリとウィジェット・ファクトリも自動的に選択されます。また、ダウンロード・ページから、「`UI lightness`」テーマがデフォルトで選択され、ダウンロード・パッケージに含められます。link:#usingcss[次の項で]このテーマを連絡先リストに適用します。
...
4. 次に、上記のコード・フラグメントで参照されているJPG人物写真を追加します。link:#requiredSoftware[先にダウンロードしたプロジェクト・リソース]から`pix`ディレクトリを取出し、ディレクトリ全体をプロジェクト・フォルダにコピーして`index.html`と同じレベルに置きます。まもなく、NetBeansで「プロジェクト」ウィンドウが自動的に更新され、新しいディレクトリがプロジェクトに手動で追加されたことが反映されます。
5. ブラウザに切り替え、ページをリフレッシュします。
image:structured-list.png[title="ブラウザに表示された構造化リスト"]
...
7. 3つのスクリプトをコピーし([Ctrl]-[C]、Macの場合は[⌘]-[C])、IDEに戻り、`jqproject`フォルダ内にlink:#js[先に作成した]`js`フォルダに貼り付けます。
...
image:accordion-list.png[title="ユーザーのクリックを処理し、アコーディオン効果を生成するアコーディオン・ウィジェット"]
...
これを行うには、プロジェクト・ノードを右クリックし、「新規」>「フォルダ」を選択します。(「フォルダ」がオプションとして表示されない場合は、IDEのツールバーにある「新規ファイル」(image:new-file-btn.png[])ボタンをクリックし、新規ファイル・ウィザードで「その他」>「フォルダ」を選択します。)フォルダに`css`という名前を付け、`index.html`ファイルと同じディレクトリ内に置きます。
...
|image:proj-win-php.png[title="プロジェクトに挿入されたjQueryデフォルト・テーマ"] |image:proj-win-java.png[title="プロジェクトに挿入されたjQueryデフォルト・テーマ"] 
|===
...
image:ui-lightness-theme.png[title="jQueryデフォルト・テーマで拡張されたアコーディオン・ウィジェットの外観"]
