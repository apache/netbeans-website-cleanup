...
= quickstart-webapps-spring_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  quickstart-webapps-spring_zh_CN
:description: Apache NetBeans  quickstart-webapps-spring_zh_CN
:toc: left
:toc-title:
...
* link:#setting[建立具有 Spring Web MVC 支持的新项目]
* link:#creating[创建 Spring Web MVC 框架项目]
* link:#running[运行框架项目]
* link:#overview[应用程序概述]
* link:#service[实现服务]
* link:#controller[实现控制器和模型]
* link:#view[实现视图]
* link:#seeAlso[另请参见]
...
image:frameworks-window.png[title=""Frameworks"（框架）面板中显示的 Spring Web MVC"]
...
image:spring-configuration.png[title="在 "Configuration"（配置）标签下指定 Spring 分派程序 Servlet 的名称和映射"]
...
image:spring-libraries.png[title="Spring JAR 在项目的 "Libraries"（库）节点下列出"]

==== 运行框架项目

在对项目文件进行任何更改之前，尝试在 IDE 中运行新项目：

1. 单击 IDE 主工具栏中的 "Run Project"（运行项目）(image:run-project-btn.png[])。IDE 将自动启动 GlassFish Server（如果尚未运行），编译项目，然后将其部署到该服务器。请注意 IDE "Output"（输出）窗口（Ctrl-4 组合键；在 Mac 上为 ⌘-4 组合键）中显示的任何输出。构建的输出以`成功构建`消息结尾。
image:output.png[title="运行项目时 "Output"（输出）窗口将显示信息"]
IDE 的默认浏览器将启动，并且您会看到欢迎页视图 (`/WEB-INF/jsp/index.jsp`) 的内容。
image:browser-output.png[title="欢迎页输出显示在浏览器中"]
...
1. 在 IDE 的工具栏中单击 "New File"（新建文件）(image:new-file-btn.png[]) 按钮。（也可以按 Ctrl-N 组合键；在 Mac 上按 ⌘-N 组合键。）
...
image:simple-form-controller.png[title="NetBeans IDE 为各种 Spring 工件提供模板"]
...
image:comment-out.png[title="突出显示代码片段，然后按 Ctrl-/ 以开启/关闭注释"]
...
image:set-command-class.png[title="为 setCommandClass() 显示错误标记"]
...
image:generate-code.png[title="通过 "Generate Code"（生成代码）弹出式菜单可以设置存取方法"]
...
image:fix-imports70.png[title="按 Ctrl-Shift-I 修复文件中的导入"]
...
image:code-completion.png[title="按 Ctrl-空格键时调用的代码完成"]
...
----
4. 采用与link:#create-jsp[以上所述]相同的方式创建另一个 JSP 页，但将其命名为 `nameView`。
...
8. 更改应用程序的相对入口点。当前，项目入口点仍然为 `index.htm`，如上面的link:#running[运行框架项目]中所述，它重定向到 `WEB-INF/jsp/index.jsp`。当部署和运行项目时，可以指定项目的入口点。在 "Projects"（项目）窗口中，右键单击该项目节点并选择 "Properties"（属性）。此时将显示 "Project Properties"（项目属性）对话框。在 "Categories"（类别）下，选择 "Run"（运行）。在 "Relative URL"（相对 URL）字段中，键入 `/hello.htm`，然后单击 "OK"（确定）。
...
image:name-view.png[title="nameView 显示在浏览器中"]
在文本字段中输入您的名字，然后按 Enter 键。将显示带有问候消息的 `helloView`：
image:hello-view.png[title="helloView 显示在浏览器中"]
