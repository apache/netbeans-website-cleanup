...
:keywords: Apache NetBeans  cvs
:description: Apache NetBeans  cvs
:toc: left
:toc-title:
...
* link:#opening[Opening a CVS Project in the IDE]
...
*Note:* Make sure your link:#fileStatus[file status labels] are activated (choose View > Show Versioning Labels from the main menu).
...
Click Merge. The IDE incorporates the branch into the trunk. If any merge conflicts arise in the process, the project's status is updated to link:#resolving[Merge Conflict] to indicate this.
