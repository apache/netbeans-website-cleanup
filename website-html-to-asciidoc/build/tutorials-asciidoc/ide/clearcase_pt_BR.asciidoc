...
:keywords: Apache NetBeans  clearcase_pt_BR
:description: Apache NetBeans  clearcase_pt_BR
:toc: left
:toc-title:
...
* link:#settingUp[Configurando o ClearCase]
* link:#specifying[Especificando o Caminho para o] Executável da `cleartool`
* link:#starting[Começando a Trabalhar com Arquivos com Controle de Versão]
* link:#opening[Abrindo um Projeto Existente no IDE]
* link:#addingSourceControl[Criando um Novo Projeto e Colocando-o no Controle de Código-fonte]
...
* link:#checkingin[Fazendo Check-in de Códigos-fonte]
...
Quando um projeto com controle de versão é aberto no IDE, todos os nomes de pasta e de arquivo são renderizados em verde, indicando que eles têm o status 'novo'. (Quaisquer arquivos exibidos em cinza possuem o status 'ignorado'. Consulte link:#badges[Emblemas e Codificação de Cor] abaixo para obter mais informações.) O IDE decide automaticamente para cada arquivo de projeto com controle de versão se ele é um candidato para controle de versão ou não (ou seja, arquivos de códigos-fonte em pacotes são geralmente com controle de versão, enquanto os arquivos privados do projeto são geralmente ignorados).
...
2. Digite uma mensagem na área de texto Descrevendo a Mensagem. Se preferir, clique no ícone de Mensagens Recentes (image:recent-msgs.png[]) localizado no canto superior direito para ver e selecionar de uma lista de mensagens que você usou anteriormente existente.
...
A funcionalidade Fazer Check-out sob Demanda pode ser ajustada por meio da opção Checkout sob demanda na link:#ccOptions[caixa de diálogo Opções do ClearCase].
...
* link:#viewingChanges[Exibindo Alterações no Editor de Código-fonte]
* link:#viewingFileStatus[Exibindo Informações de Status do Arquivo]
* link:#comparing[Comparando Versões de Arquivo]
...
image:badge-example.jpg[title="emblema de controle de versão azul exibido na janela Favoritos"]
...
|*Azul* |image:blue-text.png[] |Indica que o arquivo sofreu check-out. 

|*Verde* |image:green-text.png[] |Indica que o arquivo é novo e ainda não foi adicionado ao controle de código-fonte. 

|*Cinza* |image:gray-text.png[] |Indica que o arquivo será ignorado pelo ClearCase e não será incluído em comandos de controle de versão (ou seja, Adicionar ao Controle de Códigos-Fonte ou Check-in). Os arquivos só podem ser ignorados caso ainda não tenham tido controle de versão. 

|*Tachado* |image:strike-through-text.png[] |Indica que o arquivo será excluído de check-ins ou de operações Adicionar ao Controle de Código-Fonte. O texto tachado só aparece em locais específicos, tal como a janela de Controle de Versão, a caixa de diálogo Check-in e a caixa de diálogo Adicionar ao Controle de Códigos-fonte, quando você opta por excluir arquivos individuais de uma ação. Tais arquivos ainda são afetados por outros comandos do ClearCase, tais como Atualizar. 
|===
...
2. Selecione o botão Diversos na parte superior da janela e clique na guia Controle de Versão abaixo dela. Garanta que ClearCase está selecionado abaixo de Sistemas de Controle de versão no painel esquerdo. (Consulte a link:#ccOptions[captura de tela] acima para referência.)
...
image:file-labels.jpg[title="os labels de arquivo são exibidos próximo aos nomes de arquivo"]
...
|image:diff.png[] |*Diferenciar Tudo* |Abre o Visualizador de Diferenciação fornecendo uma comparação lado a lado de suas cópias locais e as versões mantidas no repositório. 

|image:update.png[] |*Atualizar Todos* |Atualiza todos os arquivos selecionados. (Aplica-se somente à views de snapshot.) 

|image:commit.png[] |*Fazer Check-in de todos* |Permite que você faça check-in das alterações locais. 
|===
...
image:versioning-right-click.jpg[title="Clicar com o botão direito no menu exibe um arquivo selecionado na janela Controle de Versão"]
...
Permite que você marque o arquivo para ser excluído ao realizar um check-in.
 |image:exclude-from-checkin.jpg[title="Arquivos são marcados como excluídos na caixa de diálogo de Fazer Check-in"] 
|===
...
* link:#makeChanges[Fazer Alterações em arquivos submetidos a check-out]
...
|===
|*Substituir* (image:insert.png[]): |Insere o texto realçado das versões anteriores na versão submetida a check-out 

|*Mover Todos* (image:arrow.png[]): |Reverte a versão do arquivo check-out para o estado da versão anterior selecionada 

|*Remover* (image:remove.png[]): |Remove o texto realçado da versão atual com check-out para que ela espelhe a versão anterior 
|===
...
* Na janela Visualizador de Controle de Versão ou de Diferenciação, clique no botão Fazer Check-in de Todos (image:commit.png[]) localizado na barra de ferramentas.
...
1. Digite uma mensagem de check-in na área de texto Mensagem de Check-in. Se preferir, clique no ícone de Mensagens Recentes (image:recent-msgs.png[]) localizado no canto superior direito para ver e selecionar de uma lista de mensagens que você usou anteriormente existente.
