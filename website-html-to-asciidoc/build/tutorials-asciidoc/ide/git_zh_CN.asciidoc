...
:keywords: Apache NetBeans  git_zh_CN
:description: Apache NetBeans  git_zh_CN
:toc: left
:toc-title:
...
* link:#initialize[初始化 Git 资源库]
* link:#clone[复制 Git 资源库]
* link:#github[通过 SSH 协议从 GitHub 复制资源库]
* link:#add[将文件添加到 Git 资源库中]
* link:#editing[编辑文件]
* link:#viewChanges[在源代码编辑器中查看更改]
* link:#viewFileStatus[查看文件状态信息]
* link:#diff[比较文件版本]
* link:#revert[还原更改]
* link:#committing[将源文件提交到资源库]
* link:#branch[使用分支]
* link:#branchCreate[创建分支]
* link:#branchCheckOut[检出分支]
* link:#branchMerge[合并]
* link:#branchDelete[删除分支]
* link:#remote[使用远程资源库]
* link:#fetch[获取]
* link:#pull[提取]
* link:#push[推入]
...
所有项目文件在工作树中都标记为 `Added`（已添加）。要查看文件link:#viewFileStatus[状态]，请在 "Projects"（项目）窗口中将光标放在文件名上。工作树中的文件状态以绿色显示在斜杠右侧，如下图所示。

image:new-locally.png[]

在初始化 Git 资源库后，您可以link:#add[添加]文件，也可以直接将文件link:#committing[提交]到 Git 资源库。
...
4. 在 "Commit"（提交）对话框中，选择 "Changes between HEAD and Index"（HEAD 和索引之间的更改）(image:changes-head-index.png[]) 开启/关闭按钮。
此时将显示已缓存的文件列表。
5. 按下面的link:#committing[将源文件提交到资源库]部分所述，提交这些文件。
 |

1. 在 "Projects"（项目）窗口中，右键单击要提交的文件。
2. 在上下文菜单中，选择 `Git` > `Commit`（提交）。
3. 在 "Commit"（提交）对话框中，选择 "Changes between Index and Working Tree"（索引和工作树之间的更改）(image:changes-head-wt.png[]) 开启/关闭按钮。
此时将显示未缓存的文件列表。
4. 按下面的link:#committing[将源文件提交到资源库]部分所述，提交这些文件。
 
|===

*注：*HEAD 中的文件link:#viewFileStatus[状态]以绿色显示在斜杠左侧，如下图所示。
...
在使用 "Projects"（项目）（Ctrl-1 组合键）、"Files"（文件）（Ctrl-2 组合键）、"Favorites"（收藏夹）（Ctrl-3 组合键）或 "Versioning"（版本控制）视图时，IDE 将提供一些可视化功能以帮助查看文件状态信息。在下面的示例中，请注意标记（例如，image:blue-badge.png[]）、文件名颜色和相邻状态标签如何全都彼此一致，以向您提供一种简单而有效的方法来跟踪文件的版本控制信息：
...
* link:#versioningView[Git 的 "Versioning"（版本控制）视图]
...
|*蓝色标记* (image:blue-badge.png[]) |指示工作树中存在已修改、添加或删除的文件。对于包，此标记仅应用于包本身，而不应用于它的子包。对于项目或文件夹，此标记指示其中的更改，或指示其所包含子文件夹中的任何更改。 

|*红色标记* (image:red-badge.png[]) |标记包含_冲突_文件的项目、文件夹或包。对于包，此标记仅应用于包本身，而不应用于它的子包。对于项目或文件夹，此标记指示其中的冲突，或指示其所包含子文件夹中的任何冲突。 
|===
...
|*没有特定颜色（黑色）* |image:black-text.png[] |表示未更改文件。 

|*蓝色* |image:blue-text.png[] |表示在本地修改了文件。 

|*绿色* |image:green-text.png[] |表示在本地添加了文件。 

|*红色* |image:red-text.png[] |表示文件存在合并冲突。 

|*灰色* |image:gray-text.png[] |表示文件被 Git 忽略，并且不包含在版本控制命令（例如 "Update"（更新）和 "Commit"（提交））中。如果文件已进行版本控制，则无法被忽略。 
|===
...
|image:changes-head-wt.png[] |*HEAD 和工作树之间的更改* |显示已缓存或只修改/创建而未缓存的文件列表。 

|image:changes-head-index.png[] |*HEAD 和索引之间的更改* |显示缓存的文件列表。 

|image:changes-index-wt.png[] |*索引和工作树之间的更改* |显示在已缓存和工作树状态之间存在差异的文件。 

|image:refresh.png[] |*刷新状态* |刷新选定文件和文件夹的状态。可以刷新 "Versioning"（版本控制）视图中显示的文件以反映可能在外部进行的任何更改。 

|image:open-diff.png[] |*打开比较* |打开比较查看器，您可以用它对本地副本和资源库中保留的版本进行并排比较。 

|image:update.png[] |*还原修改* |显示link:#revertdialog[还原修改]对话框。 

|image:commit-button.png[] |*提交更改* |显示link:#commitdialog[提交]对话框。 
|===
...
比较查看器使用link:#color-coding-table[颜色编码]显示版本控制更改，该颜色编码与其他地方使用的颜色编码相同。在上面显示的屏幕快照中，绿色块指示已添加到较新修订版本中的内容。红色块指示从较新修订版本中删除了以前的修订内容。蓝色指示在突出显示的行中发的更改。
...
|image:nextdiff.png[] |*转至下一个差异* |显示文件中的下一个差异。 

|image:prevdiff.png[] |*转至上一个差异* |显示文件中的上一个差异。 

|image:refresh.png[] |*刷新状态* |刷新选定文件和文件夹的状态。可以刷新 "Versioning"（版本控制）窗口中显示的文件，以反映可能已在外部执行的任何更改。 

|image:update.png[] |*还原修改* |显示 link:#revertdialog[Revert Modifications]（还原修改）对话框。 

|image:commit-button.png[] |*提交更改* |显示 link:#commitdialog[Commit]（提交）对话框。 
|===
...
|image:insert.png[] |*替换* |将突出显示的文本插入到工作树副本中。 

|image:arrow.png[] |*全部移动* |还原整个本地工作树副本。 

|image:remove.png[] |*删除* |从本地工作树副本中删除突出显示的文本。 
|===
...
IDE 将使用上面的link:#three[步骤 3] 中指定的文件替换选定的文件。
...
* 单击右上角的 `Recent Messages`（近期的消息）(image:recent-msgs.png[]) 图标，以便在以前使用过的消息列表中进行查看和选择。
* 单击右上角的 `Load Template`（加载模板）(image:msg-template.png[]) 图标，以便选择消息模板。
...
* link:#branchCreate[创建]
* link:#branchCheckOut[检出]
* link:#branchMerge[合并]
* link:#branchDelete[删除]
...
*注：*如果发生合并冲突，则使用link:#badges[红色标记]标记冲突文件，以说明这种情况。

*注：*合并之后，还必须link:#committing[提交]更改，以便将其添加到 HEAD 中。
