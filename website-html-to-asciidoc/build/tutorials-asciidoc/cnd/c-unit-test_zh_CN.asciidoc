...
:keywords: Apache NetBeans  c-unit-test_zh_CN
:description: Apache NetBeans  c-unit-test_zh_CN
:toc: left
:toc-title:
...
|- |link:#cunit[安装 CUnit 测试框架] 

|- |link:#project[为教程创建项目] 

|- |link:#addtest[将 CUnit 测试添加到 NetBeans 管理的项目中] 

|- |link:#runtest[运行 CUnit 测试] 

|- |link:#addmore[添加其他 CUnit 测试] 

|- |link:#debug[调试测试] 

|- |link:#addsimple[添加简单测试] 

|- |link:#editsimple[编辑简单测试] 

|- |link:#commandline[从命令行运行测试] 
|===
...
* link:#linux[如何在 Linux 或 Mac OS 上安装 CUnit]
* link:#solaris[如何在 Oracle Solaris 11 上安装 CUnit]
* link:#mingw[如何在 Windows 和 MinGW 上安装 CUnit]
* link:#cygwin[如何在 Windows 和 Cygwin 上安装 CUnit]
...
"make install" 完成后，即可在 IDE 中使用 CUnit 测试框架，并且您可以继续link:#project[为教程创建项目]。
...
"make install" 完成后，即可在 IDE 中使用 CUnit，并且您可以继续link:#project[为教程创建项目]。
