...
:keywords: Apache NetBeans  c-unit-test_ja
:description: Apache NetBeans  c-unit-test_ja
:toc: left
:toc-title:
...
|-  |link:#cunit[CUnitテスト・フレームワークのインストール] 

|-  |link:#project[チュートリアル用のプロジェクトの作成] 

|-  |link:#addtest[CUnitテストのNetBeans管理対象プロジェクトへの追加] 

|-  |link:#runtest[CUnitテストの実行] 

|-  |link:#addmore[別のCUnitテストの追加] 

|-  |link:#debug[テストのデバッグ] 

|-  |link:#addsimple[簡易テストの追加] 

|-  |link:#editsimple[簡易テストの編集] 

|-  |link:#commandline[コマンドラインからのテストの実行] 
|===
...
* link:#linux[LinuxまたはMac OSでのCUnitのインストール手順]
* link:#solaris[Oracle Solaris 11でのCUnitのインストール手順]
* link:#mingw[WindowsおよびMinGWでのCUnitのインストール手順]
* link:#cygwin[WindowsおよびCygwinでのCUnitのインストール手順]
...
'make install'が終了すると、CUnitテスト・フレームワークをIDEで使用する準備が整い、link:#project[チュートリアル用のプロジェクトの作成]に進むことができます。
...
'make install'が終了すると、CUnitをIDEで使用する準備が整い、link:#project[チュートリアル用のプロジェクトの作成]に進むことができます。
