...
:keywords: Apache NetBeans  html5-js-support_ja
:description: Apache NetBeans  html5-js-support_ja
:toc: left
:toc-title:
...
* link:#createproject[NetBeans HTML5アプリケーションの作成]
* link:#debugger[JavaScriptデバッガの使用]
* link:#unittests[JS単体テストの実行]
* link:#karmatests[Karmaを使用してテストを実行する方法]
* link:#jstestdriver[JSテスト・ドライバを使用してテストを実行する方法]
* link:#debugtest[JSテスト・ドライバを使用したJS単体テストのデバッグ]
* link:#summary[サマリー]
* link:#seealso[関連項目]
...
image:html5-js-newproject.png[title="新規ファイル・ウィザードの「AngularJSの端末カタログ・チュートリアル」サンプル・プロジェクト"]
...
image:html5-js-projectswindow.png[title="「プロジェクト」ウィンドウの「AngularJSの端末カタログ・チュートリアル」プロジェクト・ノード"]
...
image:html5-js-runproject1.png[title="ブラウザ・ウィンドウの「AngularJSの端末カタログ・チュートリアル」アプリケーション"]
...
image:html5-js-breakpoint1.png[title="エディタで設定されたブレークポイント"]

「ウィンドウ」→「デバッグ」→「ブレークポイント」を選択して「ブレークポイント」ウィンドウを開き、プロジェクトで設定したブレークポイントを表示できます。

image:html5-js-breakpoint2.png[title="「ブレークポイント」ウィンドウのブレークポイントのリスト"]
...
image:karma-new-config.png[title="新規ファイル・ウィザードの新規Karma構成ファイル"]
...
image:karma-plugins.png[title="エディタ内のKarma構成ファイル"]
...
image:karma-properties-window.png[title="プロジェクト・プロパティ・ウィンドウ内の「Karma」カテゴリ"]
...
image:karma-chrome.png[title="Chromeブラウザ・ウィンドウでのKarmaサーバーのステータス"]

「出力」ウィンドウで、サーバーのステータスを確認できます。不足するプラグインをインストールするよう求めるメッセージも表示されます。

image:karma-output1.png[title="「サービス」ウィンドウの「jsTestドライバの構成」ノード"]

*注意:*単体テストを実行するには、ブラウザ・ウィンドウが開き、Karmaサーバーが稼働している必要があります。

11. 「Karma」ノードを右クリックして「構成を設定」>「`karma.conf.js`」の順に選択し、正しい構成ファイルが選択されていることを確認する必要があります。image:karma-node.png[title="「サービス」ウィンドウの「jsTestドライバの構成」ノード"]
...
image:karma-test-results.png[title="Karmaテスト結果"]
...
image:html5-js-testdriver-serviceswindow.png[title="「サービス」ウィンドウの「jsTestドライバの構成」ノード"]
...
image:html5-js-testdriver-configure.png[title="「jsTestドライバの構成」ダイアログ・ボックス"]
...
image:html5-js-testdriver-configfile.png[title="新規jsTestDriver構成ファイル・ウィザード"]
...
image:html5-js-testdriver-projectswindow.png[title="「プロジェクト」ウィンドウの「単体テスト」フォルダ"]
...
image:html5-js-testdriver-browserwindow.png[title="ブラウザ・ウィンドウで稼働しているjsTestDriver"]
...
image:html5-js-testdriver-outputstatus.png[title="「出力」ウィンドウのjs-test-driverサーバー・タブ"]
...
image:html5-js-testdriver-outputwindow.png[title="「出力」ウィンドウの実行中のJS単体テスト・タブ"]
11. メイン・メニューの「ウィンドウ」→「出力」→「テスト結果」を選択して、「テスト結果」ウィンドウを開き、テストの結果を確認します。
image:html5-js-testdriver-testresultswindow.png[title="「テスト結果」ウィンドウ"]
...
image:html5-js-testdriver-testresultswindow-fail.png[title="「テスト結果」ウィンドウの失敗したテスト"]

テストを実行すると、期待される値"age"のかわりに値"name"が検出されたことを示すメッセージでテストの1つが失敗したことを確認できます。

5. 「出力」ウィンドウの実行中のJS単体テスト・タブを開きます。
image:html5-js-testdriver-outputwindow-fail.png[title="「出力」ウィンドウの「実行中のJS単体テスト」タブの失敗したテスト"]
...
image:html5-js-testdriver-evaluate.png[title="エディタ、「コードを評価」ウィンドウおよび「変数」ウィンドウを表示しているIDE"]

または、メイン・メニューの「デバッグ」→「式の評価」を選択して、「コードを評価」ウィンドウを開くことができます。ウィンドウに式`scopeOrder.prop`を入力し、「コード・フラグメントを評価(Ctrl+Enter)」ボタン(image:evaluate-button.png[title="「式の評価」ボタン"])をクリックすると、デバッガによって式の値が「変数」ウィンドウに表示されます。
