...
= configure-php-environment-mac-os_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  configure-php-environment-mac-os_zh_CN
:description: Apache NetBeans  configure-php-environment-mac-os_zh_CN
:toc: left
:toc-title:
...
* link:#installMamp[安装和配置 MAMP]
* link:#mampMySQL[注册 MAMP 的 MySQL 数据库]
* link:#phpProject[创建 PHP 项目]
* link:#installEnableXdebug[使用 MAMP 启用 Xdebug]
* link:#debug[调试 PHP 项目]
* link:#seeAlso[另请参见]
...
image:mamp-install.png[title="显示 MAMP 和 MAMP-Pro 选项的 MAMP 安装程序面板"]
...
image:mamp-control-panel-preferences-ports.png[title="在端口已更改为默认值之后带有 &quot;Ports&quot;（端口）标签的 MAMP 控制面板"]
...
image:mysql-basic-properties.png[title="包含 MAMP 默认连接设置的 &quot;MySQL Basic Properties&quot;（MySQL 基本属性）对话框"]
...
image:mysql-admin-props.png[title="包含样例管理设置的 &quot;MySQL Admin Properties&quot;（MySQL 管理属性）对话框"]
...
例如，创建新的 PHP 项目时，使用新建项目向导（在 IDE 的工具栏中单击 "New Project"（新建项目）图标）(image:new-project-icon.png[])。在此向导的第三步 "Run Configuration"（运行配置）中，选择 "Copy files from Sources Folder to anotherlocation"（将源文件夹中的文件复制到其他位置）选项，并键入 `htdocs` 文件夹的路径。

link:new-php-project.png[image:new-php-project.png[title="输入 Apache htdocs 文件夹的路径以允许 IDE 将源复制到其已部署位置"]]
...
image:php-options68.png[title="可以在 &quot;PHP Options&quot;（PHP 选项）窗口中设置调试端口"]
如果需要，您可以在此处更改调试器端口。
5. 打开 MAMP 控制面板并选择 "PHP" 标签。取消选择 "Zend Optimizer"（Zend 优化器）。
image:mamp-control-panel-preferences-php.png[title="取消选择 &quot;Zend Optimizer&quot;（Zend 优化器）之后带有 &quot;PHP&quot; 标签的 MAMP 控制面板"]
6. 启动（或重新启动）MAMP Apache 服务器。

=== 调试 PHP 项目

要在 IDE 中调试 PHP 项目，请在 "Projects"（项目）窗口中右键单击该项目并选择 "Debug"（调试）。或者，如果在 "Projects"（项目）窗口中突出显示了该项目，可以单击主工具栏中的 "Debug Project"（调试项目）图标 (image:debug-icon.png[])。

通过在 link:#phpOptions["PHP Options"（PHP 选项）窗口]中启用此选项，可以将调试器设置为在第一行代码处挂起。

当调试器会话处于活动状态时，将在编辑器上方显示调试器工具栏。

image:debugger-toolbar.png[title="处于挂起状态的调试器工具栏"]

您还可以通过打开 "Sessions"（会话）窗口来验证 PHP 调试会话是否处于活动状态。从主菜单中选择 "Window"（窗口）> "Debugging"（调试）> "Sessions"（会话）。

image:debugger-sessions-win.png[title="&quot;Sessions&quot;（会话）窗口指示 Xdebug 调试器会话处于活动状态"]
