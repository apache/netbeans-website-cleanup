...
:keywords: Apache NetBeans  phpunit_zh_CN
:description: Apache NetBeans  phpunit_zh_CN
:toc: left
:toc-title:
...
* link:#installing-phpunit[安装 PHPUnit]
* link:#create-test[创建并运行 PHPUnit 测试]
* link:#test-groups[使用测试组]
* link:#result-windows[测试结果和 IDE 输出]
* link:#code-coverage[代码覆盖率]
* link:#project-specific-configurations[使用项目特定的配置]
* link:#selenium[在 Selenium 框架上运行测试]
...
5. 对于方法 `testAdd2` 和 `testAdd4`，添加标注 `@group production` 和 `@group development`。image:production-development-group-code.png[]
...
* 重新运行测试按钮 image:rerun-button.png[]
* 显示失败的测试 image:show-failed.png[]
* 显示通过的测试 image:show-passed.png[]
* 显示通过但出错的测试 image:show-error.png[]
* 导航至下一个测试结果的按钮 image:next-test-button.png[]，以及导航至上一个测试结果的按钮 image:previous-test-button.png[]

"Output"（输出）窗口显示 PHPUnit 脚本的完整输出。当您无法通过 "Test Results"（测试结果）窗口中的信息找出错误原因时，该窗口会非常有用。与 "Test Results"（测试结果）窗口类似，"Output"（输出）窗口包括指向失败的测试类行的链接。它还在左侧包括用于重新运行测试和用于打开 "PHP Options"（PHP 选项）窗口的按钮。image:options-link-button.png[]
