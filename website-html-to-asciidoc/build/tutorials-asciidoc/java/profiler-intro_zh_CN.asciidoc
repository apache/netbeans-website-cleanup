...
= profiler-intro_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  profiler-intro_zh_CN
:description: Apache NetBeans  profiler-intro_zh_CN
:toc: left
:toc-title:
...
* link:#firsttime[首次执行性能分析]
* link:#create[创建样例项目]
* link:#calibrate[首次使用分析器]
* link:#select[选择性能分析任务]
* link:#monitor[监视应用程序]
* link:#cpu[分析应用程序性能]
* link:#memory[分析内存使用情况]
* link:#snapshot[生成快照]
* link:#takesnap[生成和比较内存快照]
...
image:calibrate-select-platform.png[title="用于在校准时选择 Java 平台的对话框。"]

完成校准操作之后，将会显示一个对话框。您可以单击 "Show Details"（显示详细信息），以查看一个对话框，其中包含与校准结果有关的信息。每个 Java 平台的校准数据都保存在主目录的 `.nbprofile` 目录中。

image:calibrate-information.png[title="&quot;Calibration Information&quot;（校准信息）对话框"]
...
|link:#monitor[监视应用程序] |选择此任务可获取有关目标 JVM 属性的高级信息，其中包括线程活动和内存分配。 

|link:#cpu[分析 CPU 性能] |选择此任务可获取有关应用程序性能的详细数据，其中包括方法的执行时间和调用次数。 

|link:#memory[分析内存使用情况] |选择此任务可获取有关对象分配和垃圾回收的详细数据。 
|===
...
image:monitor-task.png[title="选择 &quot;Monitor Application&quot;（监视应用程序）性能分析任务"]
...
image:profile-intro-telemoverview.png[title="&quot;Telemetry Overview&quot;（遥测概览）窗口"]
...
image:select-cpu-task.png[title="选择 CPU 性能分析任务"]
...
image:edit-profiling-roots.png[title="用于选择根方法的对话框"]
7. 单击 "Advanced"（高级）按钮，以打开 "Edit Profiling Roots (Advanced)"（编辑性能分析根目录（高级））对话框，该对话框提供了更高级的选项，用于添加、编辑和删除根方法。
image:edit-profiling-roots-adv.png[title="用于指定根方法的对话框"]
...
image:show-filter-value.png[title="&quot;Show Filter Value&quot;（显示过滤器值）对话框"]
...
image:cpu-liveresults1.png[title="CPU Live Results（CPU 实时结果）"]
...
image:profile-java-memory.png[title="选择内存性能分析任务"]
...
image:profile-java-memresults1.png[title="&quot;Memory Profiling Results&quot;（内存性能分析结果）"]
...
image:profile-java-memstack1.png[title="内存性能分析结果的屏幕快照"]
...
5. 在某一内存快照的窗口中，单击快照工具栏中的 "Compute Difference"（计算差异）按钮 (image:comparesnapshots.png[title="&quot;Compare Snapshot&quot;（比较快照）按钮"])。此外，也可以从主菜单中选择 "Profile"（性能分析）> "Compare Memory Snapshots"（比较内存快照）。
6. 在 "Select Snapshot to Compare"（选择要比较的快照）中，从列表中选择其中一个打开的快照。单击 "OK"（确定）。
image:profile-java-selectsnap.png[title="&quot;Select Snapshots to Compare&quot;（选择要比较的快照）对话框"]
...
image:profile-java-compare.png[title="显示两个内存快照的活动比较结果的标签"]
