...
:keywords: Apache NetBeans  gui-binding_zh_CN
:description: Apache NetBeans  gui-binding_zh_CN
:toc: left
:toc-title:
...
image:netbeans-stamp-80-74-73.png[title="此页上的内容适用于 NetBeans IDE 7.1、7.2、7.3、7.4 和 8.0"]

* link:#intro[简介：NetBeans IDE 中的 Bean 绑定]
* link:#prop-binding[将属性绑定到其他属性]
* link:#prop-custom[绑定定制 Bean]
* link:#prop-advanced[高级绑定属性]
* link:#data-binding[将数据绑定到组件]
* link:#prop-synth[特殊绑定属性]
...
*注：*绑定可以是双向的（读/写），这样，在目标中所做的更改可以自动地反映在源中。但是，初始绑定方向始终从源到目标。请参见link:#prop-advanced[高级绑定配置]一节中有关“更新模式”的信息。
