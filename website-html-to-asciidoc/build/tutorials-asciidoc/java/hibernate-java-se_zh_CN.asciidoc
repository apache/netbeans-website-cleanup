...
= hibernate-java-se_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  hibernate-java-se_zh_CN
:description: Apache NetBeans  hibernate-java-se_zh_CN
:toc: left
:toc-title:
...
* link:#01[创建数据库]
* link:#02[创建 Java Swing 应用程序项目]
* link:#03[在项目中添加 Hibernate 支持]
* link:#03a[创建 Hibernate 配置文件]
* link:#03b[修改 Hibernate 配置文件]
* link:#03c[创建 `HibernateUtil.java` 帮助文件]
* link:#05[生成 Hibernate 映射文件和 Java 类]
* link:#05a[创建逆向工程文件]
* link:#05b[通过数据库创建 Hibernate 映射文件和 POJO]
* link:#06[创建应用程序 GUI]
...
image:hib-libraries-config.png[title="显示 Hibernate 库的 "Projects"（项目）窗口的屏幕快照"]
...
image:hib-util.png[title="显示如何创建 HibernateUtil 的新建文件向导"]
...
* 要为多个表创建文件时，您很可能希望使用这些向导。在本教程中，您只需创建一个 POJO 和一个映射文件，因此单独创建各文件相当简单。本教程的末尾介绍了link:#10[单独创建 POJO 和映射文件]的步骤。
...
image:mapping-pojos-wizard-ant.png[title=""Generate Hibernate Mapping Files and POJOs"（生成 Hibernate 映射文件和 POJO）向导"]
