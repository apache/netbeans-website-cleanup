...
= maven-hib-java-se_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  maven-hib-java-se_zh_CN
:description: Apache NetBeans  maven-hib-java-se_zh_CN
:toc: left
:toc-title:
...
* link:#01[创建数据库]
* link:#02[配置 Maven]
* link:#02a[查看 Maven 资源库]
* link:#03[创建 Maven 应用程序]
* link:#03b[修改 Maven 项目属性]
* link:#04[添加 Hibernate 文件和依赖关系]
* link:#04a[创建 Hibernate 配置文件]
* link:#04b[修改 Hibernate 配置文件]
* link:#04c[创建 `HibernateUtil.java` 帮助文件]
* link:#05[生成 Hibernate 映射文件和 Java 类]
* link:#05a[通过数据库创建 Hibernate 映射文件和 POJO]
* link:#06[创建应用程序 GUI]
* link:#06a[创建 JFrame 窗体]
* link:#06b[在窗体中添加元素]
* link:#07[在 HQL 查询编辑器中创建查询]
* link:#08[在窗体中添加查询]
* link:#09[运行 Maven 项目]
* link:#09a[下载解决方案项目]
* link:#10[单独创建 POJO 和映射文件]（可选）
...
* link:http://books.sonatype.com/mvnref-book/reference/introduction.html[Chapter 1. Introducing Apache Maven]（第 1 章. Apache Maven 简介，来自 link:http://books.sonatype.com/mvnex-book/reference/public-book.html[Maven by Example]（Maven，按示例））
...
image:maven-repositories.png[title=""Services"（服务）窗口中的 "Maven Repositories"（Maven 资源库）"]
...
image:maven-project-wizard.png[title="新建项目向导中的 "Maven Archetypes"（Maven 原型）"]
...
image:hib-config.png[title="用于选择数据库连接的对话框"]
...
image:maven-project-libs.png[title=""Projects"（项目）窗口中的 "Libraries"（库）节点下的依赖关系"]
...
image:maven-artifacts-viewer.png[title="显示依赖关系的 "Graphs"（图形）标签或工件查看器"]
...
image:add-property-factoryclass-4.png[title="显示 hibernate.query.factory_class 属性的设置值的 "Add Hibernate Property"（添加 Hibernate 属性）对话框"]
...
image:maven-hibutil-wizard.png[title="Hibernate 实用程序向导"]
...
*注：*要为多个表创建文件时，您很可能希望使用这些向导。在本教程中，您只需创建一个 POJO 和一个映射文件，因此单独创建各文件相当简单。本教程的末尾介绍了link:#10[单独创建 POJO 和映射文件]的步骤。
...
image:mapping-pojos-wizard.png[title=""Generate Hibernate Mapping Files and POJOs"（生成 Hibernate 映射文件和 POJO）向导"]
...
image:hib-jframe-form.png[title="编辑器设计视图中的 GUI 窗体"]
...
4. 通过在 HQL 查询编辑器中键入 `from Actor` 来测试连接。单击工具栏中的 "Run HQL Query"（运行 HQL 查询）按钮 (image:run_hql_query_16.png[title=""Run HQL Query"（运行 HQL 查询）按钮"])。

单击 "Run HQL Query"（运行 HQL 查询）按钮之后，您应该能在 HQL 查询编辑器的底部窗格中看到查询结果。

image:hib-query-hqlresults.png[title="显示 HQL 查询结果的 HQL 查询编辑器"]
...
image:browse-main-class.png[title="在 "Browse Main Classes"（浏览主类）对话框中设置主类"]
...
image:maven-projectproperties.png[title="DVDStoreAdmin 项目属性窗口的 "Actions"（操作）窗格"]
...
image:application-run.png[title="显示结果的 DVDStoreAdmin 应用程序"]
...
image:getters-setters.png[title=""Generate Getters and Setters"（生成 getter 和 setter）对话框"]
...
image:mapping-wizard.png[title="生成 Hibernate 映射文件向导"]
