...
= hibernate-java-se_ja
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  hibernate-java-se_ja
:description: Apache NetBeans  hibernate-java-se_ja
:toc: left
:toc-title:
...
* link:#01[データベースの作成]
* link:#02[Java Swingアプリケーション・プロジェクトの作成]
* link:#03[プロジェクトへのHibernateサポートの追加]
* link:#03a[Hibernate構成ファイルの作成]
* link:#03b[Hibernate構成ファイルの変更]
* link:#03c[`HibernateUtil.java`ヘルパー・ファイルの作成]
* link:#05[Hibernateのマッピング・ファイルおよびJavaクラスの生成]
* link:#05a[リバース・エンジニアリング・ファイルの作成]
* link:#05b[データベースからのHibernateのマッピング・ファイルとPOJOの作成]
...
image:hib-libraries-config.png[title="Hibernateライブラリが表示された「プロジェクト」ウィンドウのスクリーンショット"]
...
image:hib-util.png[title="HibernateUtilの作成方法を示す新規ファイル・ウィザード"]
...
* 複数の表に対するファイルを作成する場合は、このウィザードを使用すると便利です。このチュートリアルでは、POJOを1つとマッピング・ファイルを1つのみ作成する必要があるため、ファイルを個々に作成する方法が簡単です。link:#10[POJOとマッピング・ファイルを個別に作成する]手順については、このチュートリアルの最後に紹介します。
...
image:mapping-pojos-wizard-ant.png[title="「Hibernateのマッピング・ファイルとPOJOの生成」ウィザード"]
