...
:keywords: Apache NetBeans  javase-intro_ja
:description: Apache NetBeans  javase-intro_ja
:toc: left
:toc-title:
...
* link:#setup[プロジェクトの設定]
* link:#create[Javaソース・コードの作成と編集]
* link:#comp[アプリケーションのコンパイルと実行]
* link:#test[アプリケーションのテストとデバッグ]
* link:#build[アプリケーションのビルド、実行、および配布]
* link:#other-tasks[その他の共通タスク]
* link:#nextsteps[次の手順]
...
3. ユーザーに、前述のlink:#running-outside-IDE[IDEの外部でのアプリケーションの実行]の項の手順に従うように指示します。
