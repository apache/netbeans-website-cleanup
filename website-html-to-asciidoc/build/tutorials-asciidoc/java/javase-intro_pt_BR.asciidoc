...
:keywords: Apache NetBeans  javase-intro_pt_BR
:description: Apache NetBeans  javase-intro_pt_BR
:toc: left
:toc-title:
...
* link:#setup[Configuração do Projeto]
* link:#create[Criando e Editando o Código-fonte Java]
* link:#comp[Compilando e Executando a Aplicação]
* link:#test[Testando e Depurando a Aplicação]
* link:#build[Construindo, Executando e Distribuindo a Aplicação]
* link:#other-tasks[Outras Tarefas Comuns]
...
==== link:[Criando um Projeto de Biblioteca de Classe Java]
...
==== link:[Criando um Projeto de Aplicação Java]
...
==== link:[Configurando o Classpath de Compilação]
...
3. Oriente os usuários a seguir as etapas na seção link:#running-outside-IDE[Executando a Aplicação Fora do IDE] acima.
