...
= quickstart-gui_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  quickstart-gui_zh_CN
:description: Apache NetBeans  quickstart-gui_zh_CN
:toc: left
:toc-title:
...
* link:#getting_started[入门指南]
* link:#project[创建项目]
* link:#container[创建 JFrame 容器]
* link:#getting_familiar[熟悉 GUI 构建器]
* link:#key_concepts[主要概念]
* link:#design[自由设计]
* link:#snapping[自动组件定位（对齐放置）]
* link:#feedback[可视反馈]
* link:#first_things[先打基础]
* link:#adding_components[添加组件：基础]
* link:#individual[将单个组件添加到窗体]
* link:#multiple[将多个组件添加到窗体]
* link:#inserting[插入组件]
* link:#moving_forward[更进一步]
* link:#aligning_components[组件对齐]
* link:#baseline_alignment[基线对齐]
* link:#reviewing[回顾所学内容]
* link:#adding_aligning_anchoring[添加、对齐和锚定]
* link:#component_sizing[调整组件大小]
* link:#indentation[缩进]
* link:#adjusting_form[进行最后的调整]
* link:#previewing_form[预览 GUI]
* link:#deploying[部署 GUI 应用程序]
* link:#seealso[另请参见]
...
尽管 IDE 的 GUI 构建器简化了创建 Java GUI 的过程，但是在开始布局之前大体设计出界面的外观通常还是会很有帮助的。许多界面设计者将此视为一种“最佳做法”技术，但对本教程来说，只需跳转至后面的link:#previewing_form[预览 GUI] 部分，浏览一下最终窗体应具有的外观即可。
...
2. 单击工具栏中的 "Align Right in Column"（列向右对齐）按钮 (image:align_r.png[])。或者，也可以右键单击其中任一组件，然后从弹出式菜单中选择 "Align"（对齐）> "Right"（列右侧）。
...
现在，您已成功构建了 ContactEditor GUI，接下来可以尝试用该界面查看结果了。您可以在工作过程中预览窗体，方法是单击 GUI 构建器工具栏中的 "Preview Form"（预览窗体）按钮 (image:test_form.png[])。此时窗体在单独的窗口中打开，以便您在构建并运行窗体之前对其进行测试。
