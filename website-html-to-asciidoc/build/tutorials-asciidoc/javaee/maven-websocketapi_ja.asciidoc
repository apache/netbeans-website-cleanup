...
= maven-websocketapi_ja
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  maven-websocketapi_ja
:description: Apache NetBeans  maven-websocketapi_ja
:toc: left
:toc-title:
...
*注意:*このチュートリアルは、link:http://blog.arungupta.me/[Arun Gupta氏のブログ]で見ることができるブログ投稿 link:https://blogs.oracle.com/arungupta/entry/collaborative_whiteboard_using_websocket_in[Collaborative Whiteboard using WebSocket in GlassFish 4 - Text/JSON and Binary/ArrayBuffer Data Transfer (TOTD #189)]およびその他のブログ・エントリに基づいています。このブログにアクセスして、WebSocket APIやGlassFish 4の使用に関するその他の多くの有用なエントリをぜひ参照してください。
...
* link:#createendpoint[WebSocketエンドポイントの作成]
* link:#createendpoint1[エンドポイントの作成]
* link:#createendpoint2[WebSocketセッションの開始]
* link:#createendpoint3[エンドポイントのテスト]
* link:#createwhiteboard[ホワイトボードの作成]
* link:#createwhiteboard1[キャンバスの追加]
* link:#createwhiteboard2[POJOの作成]
* link:#createwhiteboard3[座標クラスの作成]
* link:#createwhiteboard6[JSON文字列の生成]
* link:#createwhiteboard4[エンコーダおよびデコーダ・インタフェースの実装]
* link:#createwhiteboard5[アプリケーションの実行]
* link:#sendbinary[エンドポイントへのバイナリ・データの送信]
...
image:websocket-newproject.png[title="新規プロジェクト・ウィザードの「サーバー」および「Java EEバージョン」"]
...
image:websocket-newendpoint.png[title="新規ファイル・ウィザードのWebSocketエンドポイント"]
...
image:websocket-endpoint-hint.png[title="ソース・エディタのコード・ヒント"]
...
image:websocket-browser1.png[title="ブラウザ・ウィンドウ内のエンドポイントへの接続メッセージ"]
...
image:websocket-generategetter.png[title="「取得メソッドおよび設定メソッドの生成」ダイアログ・ボックス"]
...
image:websocket-generateconstructor.png[title="「コンストラクタの生成」ポップアップ・メニュー"]
...
image:websocket-onebrowser.png[title="ブラウザ内の図形を含むキャンバスおよびWebコンソールに表示されたJSON"]

別のブラウザで`http://localhost:8080/WhiteboardApp/`を開くと、一方のブラウザのキャンバス内をクリックするたびに新しい円や正方形が他方のブラウザのキャンバスに複製されることがわかります。

image:websocket-twobrowsers.png[title="エンドポイントを介してJSONを送信する2つのブラウザ"]
...
image:websocket-onebrowser-binary.png[title="バイナリ・データが送信されたというメッセージを表示するブラウザ内のWebコンソール"]
