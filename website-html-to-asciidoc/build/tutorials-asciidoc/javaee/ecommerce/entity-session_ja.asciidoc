...
:keywords: Apache NetBeans  entity-session_ja
:description: Apache NetBeans  entity-session_ja
:toc: left
:toc-title:
...
* link:#whatEJB[EJBおよびJPAテクノロジとは]
* link:#whatSession[セッションBeanとは]
* link:#specification[仕様および実装について]
* link:#addEntity[エンティティ・クラスの追加]
* link:#addSession[セッションBeanの追加]
* link:#access[EJBによるデータへのアクセス]
* link:#seeAlso[関連項目]
...
image:mvc-diagram.png[title="AffableBeanアプリケーションのMVC図"]
...
* *セキュリティ:* EJBは、JAAS (Java Authentication and Authorization Service) APIとの統合をサポートしているため、アプリケーションをセキュリティ・コードで複雑にするかわりに、セキュリティを完全に外部に切り離して単純な構成でアプリケーションをセキュリティ保護できます。^link:#footnote2[[2]]^ユニット11のlink:security.html#secureEJB[アプリケーションの保護]では、EJBの<a href="http://download.oracle.com/javaee/6/api/javax/annotation/security/RolesAllowed.html" target="_blank"`@RolesAllowed`注釈を示します。
...
image:java-ee-container.png[title="GlassFish v3には、Webコンテナ、EJBコンテナおよびEclipseLink (持続性プロバイダ)がJava EEコンテナとして格納される"]
...
image:entity-from-database.png[title="使用可能なデータ・ソースの選択し、IDEでデータベースの表を読み取らせる"]
...
image:entity-classes-68.png[title="NetBeans 6.8 - データベースからのエンティティ・クラス・ウィザード、ステップ3: エンティティ・クラス"]
1. 「パッケージ」フィールドに「*entity*」と入力します。ウィザードの完了時に、エンティティ・クラスの新しいパッケージが作成されます。
2. 「持続性ユニットを作成」ボタンをクリックします。「持続性ユニットを作成」ダイアログが開きます。
image:create-pu.png[title="「持続性ユニットを作成」ダイアログを使用したpersistence.xmlファイルの生成"]
...
5. 下のlink:#step7[ステップ7]に進みます。

===== NetBeans IDE 6.9

image:entity-classes-69.png[title="NetBeans 6.9 - データベースからのエンティティ・クラス・ウィザード、ステップ3: エンティティ・クラス"]
1. 「パッケージ」フィールドに「*entity*」と入力します。ウィザードの完了時に、エンティティ・クラスの新しいパッケージが作成されます。
2. 次の点に注意してください。
...
image:entity-classes.png[title="「プロジェクト」ウィンドウでの新しいエンティティ・クラスの表示"]
...
image:embedded-id.png[title="[Ctrl]-[Space]の押下によるAPIドキュメントの呼出し"]
...
image:persistence-unit.png[title="AffableBeanPU持続性ユニットの「デザイン」ビュー"]
...
image:session-beans-for-entity-classes.png[title="「エンティティ・クラスのセッションBean」を選択し、持続性モデルのセッション・ファサードを生成する"]
3. 「次」をクリックします。
...
image:generated-session-beans.png[title="新しいセッションBeanの場所およびインタフェース作成の有無の指定"]
...
|image:projects-window-session-beans.png[title="「プロジェクト」ウィンドウでの新しいセッションBeanの調査"] |image:projects-window-session-beans-69.png[title="「プロジェクト」ウィンドウでの新しいセッションBeanの調査"] 
|===
...
* link:#index[インデックス・ページ]
* link:#category[カテゴリ・ページ]
...
image:method-signature.png[title="[Ctrl]キーを押しながらカーソルをメソッド上に移動して署名を表示する"]
...
image:override.png[title="IDEのヒントを使用した、メソッドへの@Overrides注釈の追加"]
...
image:commented-out.png[title="エディタでの[Ctrl]-[/]の押下によるコード・スニペットのコメントアウト"]
...
image:go-to-file.png[title="「ファイルに移動」ダイアログを使用した、エディタへのファイルの速やかな表示"]
10. `<resource-ref>`エントリをコメントアウト(または削除)します。このエントリは、サーバーに登録されたデータ・ソースを`<sql>`タグで識別するために必要でした。現時点では、JPAに依存してデータベースにアクセスしており、持続性ユニットに`jdbc/affablebean`データ・ソースがすでに指定されています。(上記のlink:#pu[プロジェクトの持続性ユニットの「デザイン」ビュー]を参照。)
...
----
11. プロジェクトを実行します。「プロジェクトの実行」(image:run-project-btn.png[])ボタンをクリックします。プロジェクトのインデックス・ページがブラウザで開き、4つのすべてのカテゴリ名およびイメージが表示されるのが確認できます。
image:index-page.png[title="インデックス・ページにカテゴリの詳細を取得できることの確認"]
...
* link:#categoryData[カテゴリ・データ]
* link:#selectedCategoryData[選択されたカテゴリ]
* link:#productData[選択されたカテゴリの製品データ]
...
image:comment-out-sql.png[title="カテゴリ・ページでのJSTLの<sql>文のコメントアウト"]
...
<c:forEach var="category" items="*${categories}*">
----
3. プロジェクトを実行して、カテゴリ・ページの現在の状態を調べます。「プロジェクトの実行」(image:run-project-btn.png[])ボタンをクリックします。プロジェクトのインデックス・ページがブラウザで開いたら、4つのカテゴリのいずれかをクリックします。左の列にカテゴリ・ボタンが表示され、想定したとおりに機能します。
image:category-page-left-column.png[title="左の列にカテゴリのボタンが表示され、想定したとおりに機能する"]
...
4. 左マージンにあるバッジ(image:editor-badge.png[])をクリックしてエディタのヒントを使用し、`selectedCategory`を`doGet`メソッド内のローカル変数として宣言します。
image:local-variable.png[title="エディタのヒントを使用してローカル変数を宣言する"]
...
image:category-page-selected-category.png[title="選択されたカテゴリの名前がカテゴリ・ページに表示される"]
...
image:local-variable2.png[title="エディタのヒントを使用してローカル変数を宣言する"]
...
image:category-page-product-table.png[title="製品の表に指定のカテゴリの製品が表示される"]
...
1. link:#1[^]出典: 『link:http://www.manning.com/panda/[EJB 3 In Action]』の第1章、1.1.2項: EJB as a framework
2. link:#2[^]他にもEJBによって提供される多くのサービスがあります。より総合的な一覧については、『link:http://www.manning.com/panda/[EJB 3 In Action]』の第1章、1.3.3項: Gaining functionality with EJB servicesを参照してください。
