...
= entity-session_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  entity-session_zh_CN
:description: Apache NetBeans  entity-session_zh_CN
:toc: left
:toc-title:
...
7. *添加实体类和会话 Bean*
* link:#whatEJB[什么是 EJB 和 JPA 技术？]
* link:#whatSession[什么是会话 Bean？]
* link:#specification[关于规范和实现]
* link:#addEntity[添加实体类]
* link:#addSession[添加会话 Bean]
* link:#access[使用 EJB 访问数据]
* link:#seeAlso[另请参见]
...
image:mvc-diagram.png[title="AffableBean 应用程序的 MVC 图"]
...
作为一个框架，EJB 容器将这些通用功能以快捷服务的方式提供，以便 EJB 组件可以在应用程序中使用，而不必再浪费时间做无用功。例如，假定当您在 Web 应用程序中构建信用卡模块时，编写大量复杂而又容易出错的代码来管理事务和安全访问控制。通过使用 EJB 容器提供的声明性事务和安全服务，可以避免这些事情。在将这些服务及许多其他服务部署到 EJB 容器之后，它们即可用于 EJB 组件。这意味着编写高质量、功能丰富的应用程序可能要比想象的要快许多。_^link:#footnote1[[1]]^
----
...
* *安全性：*EJB 支持与 Java 验证和授权服务 (JAAS) API 集成，因此可以使用简单配置轻松地全面呈现安全性，确保应用程序安全，而不必使用安全代码使应用程序过于混乱。^link:#footnote2[[2]]^ 第 11 单元link:security.html#secureEJB[保护应用程序]说明了 EJB 的 <a href="http://download.oracle.com/javaee/6/api/javax/annotation/security/RolesAllowed.html" target="_blank"`@RolesAllowed` 标注。
...
image:java-ee-container.png[title="作为 Java EE 容器，GlassFish v3 包含 Web 和 EJB 容器以及持久性提供器 EclipseLink"]
...
image:entity-from-database.png[title="选择可用数据源以便让 IDE 在数据库表中进行读取"]
...
image:entity-classes-68.png[title="NetBeans 6.8 -“通过数据库生成实体类”向导，步骤 3："]
...
5. 继续执行下面的link:#step7[步骤 7]。

===== NetBeans IDE 6.9

image:entity-classes-69.png[title="NetBeans 6.9 -“通过数据库生成实体类”向导，步骤 3："]
1. 在 "Package"（包）字段中键入 *entity*。在完成时，向导将为实体类创建一个新的包。
2. 注意以下几点：
...
image:entity-classes.png[title="在 "]
...
image:session-beans-for-entity-classes.png[title="选择 "]
3. 单击 "Next"（下一步）。
...
image:generated-session-beans.png[title="指定新会话 Bean 的位置以及是否创建接口"]
...
|image:projects-window-session-beans.png[title="在 "] |image:projects-window-session-beans-69.png[title="在 "] 
|===
...
image:method-signature.png[title="将鼠标悬停在方法上并同时按住 Ctrl 键可查看其签名"]
单击超链接可以直接导航到该方法。
5. 使用 IDE 的提示添加 `@Overrides` 标注。`init` 方法是通过 `HttpServlet` 的超类 `GenericServlet` 定义的。
image:override.png[title="使用 IDE 的提示向方法添加 @Overrides 标注"]
...
image:commented-out.png[title="按 Ctrl-/ 组合键可在编辑器中注释掉代码片段"]
...
10. 注释掉（或删除）`<resource-ref>` 条目。`<sql>` 标记需要使用此条目来标识服务器上注册的数据源。现在，我们依靠 JPA 访问数据库，并且已在持久性单元中指定了 `jdbc/affablebean` 数据源。（请参见上文的link:#pu[项目持久性单元的 "Design"（设计）视图]。）
...
----
11. 运行项目。单击 "Run Project"（运行项目）(image:run-project-btn.png[]) 按钮。项目的索引页在浏览器中打开，您可以看到所有四个类别名称和图像都显示出来。
image:index-page.png[title="确保索引页能够检索类别详细信息"]
...
* link:#categoryData[类别数据]
* link:#selectedCategoryData[选定的类别]
* link:#productData[选定类别的产品数据]
...
image:comment-out-sql.png[title="在类别页中注释掉 JSTL <sql> 语句"]
...
<c:forEach var="category" items="*${categories}*">
----
3. 运行项目以检查类别页的当前状态。单击 "Run Project"（运行项目）(image:run-project-btn.png[]) 按钮。当在浏览器中打开项目的索引页时，单击四个类别中的任何一个。左列中的类别按钮按预期显示和运行。
image:category-page-left-column.png[title="左栏中的类别按钮按预期显示和运行"]
...
4. 单击左旁注中的标记 (image:editor-badge.png[]) 以使用编辑器提示在 `doGet` 方法中将 `selectedCategory` 声明为一个局部变量。
image:local-variable.png[title="使用编辑器提示声明局部变量"]
...
image:category-page-selected-category.png[title="所选类别名称显示在类别页中"]
...
image:local-variable2.png[title="使用编辑器提示声明局部变量"]
...
image:category-page-product-table.png[title="产品表显示给定类别的产品"]

本单元简要地介绍了 JPA 和 EJB 技术。同时还介绍 Java 规范的角色，以及 GlassFish 应用服务器如何使用引用实现。其次，演示如何创建提供项目数据库的 Java 实现的 JPA 实体类集。然后，按照_会话 Facade_ 模式，演示如何创建 EJB 会话 Bean 集，这些 Bean 存在于实体类的顶部，并且可以更方便地对其进行访问。最后，修改 `AffableBean` 项目以利用新的会话 Bean 和实体进行索引页和类别页所需的数据库访问。
...
1. link:#1[^] 改编自link:http://www.manning.com/panda/[《EJB 3 操作》]第 1 章，1.1.2 部分：EJB 作为一个框架。
2. link:#2[^]EJB 提供许多其他的服务。有关更全面的列表，请参见link:http://www.manning.com/panda/[《EJB 3 实践》]，第 1 章，1.3.3 部分：使用 EJB 服务获得功能。
