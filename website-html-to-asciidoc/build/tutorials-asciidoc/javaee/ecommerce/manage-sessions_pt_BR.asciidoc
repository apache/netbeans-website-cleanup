...
= manage-sessions_pt_BR
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  manage-sessions_pt_BR
:description: Apache NetBeans  manage-sessions_pt_BR
:toc: left
:toc-title:
...
* link:#session-data[Tratand Dados de Sessão]
* link:#debug[Analisando os Dados de Sessão com o Depurador Java]
* link:#session-track[Analisando as Opções de Rastreamento de Sessão]
* link:#time-out[Tratando Time-Outs de Sessão]
...
* link:#httpSession[Trabalhando com um Objeto HttpSession]
* link:#scopedVariables[Trabalhando com Variáveis de Escopos em Aplicações Web]
...
1. Abra o link:https://netbeans.org/projects/samples/downloads/download/Samples%252FJavaEE%252Fecommerce%252FAffableBean_snapshot5.zip[snapshot do projeto] para esta unidade do tutorial no IDE. Clique no botão Abrir Projeto (image:open-project-btn.png[]) e use o assistente para navegar para o local em seu computador em que você fez download do projeto. Se estiver prosseguindo de uma link:entity-session.html[unidade de tutorial anterior], observe que este snapshot do projeto inclui um novo pacote `cart`, que contém as classes `ShoppingCart` e `ShoppingCartItem`. E note também que os seguintes arquivos foram modificados:
* `WEB-INF/web.xml`
* `css/affablebean.css`
...
2. Execute o projeto (image:run-project-btn.png[]) para garantir que esteja configurado corretamente com seu banco de dados e servidor de aplicações.
...
image:category-page.png[title="A página Categoria inclui a funcionalidade do carrinho de compras"]
...
image:cart-page.png[title="A página Carrinho inclui a funcionalidade do carrinho de compras"]

===== página de check-out

* Clicar em "exibir carrinho" resulta na exibição da página do carrinho.
* Clicar em "submeter compra" resulta na exibição da página de confirmação (sem dados específicos do usuário).
image:checkout-page.png[title="A página Check-out inclui a funcionalidade do carrinho de compras"]
...
image:go-to-file-dialog.png[title="Use a caixa de diálogo Ir para Arquivo para abrir rapidamente recursos do projeto no editor"]
...
image:breakpoint.png[title="Clique na margem esquerda do editor para definir pontos de interrupção"]
...
6. Execute o depurador. Clique no botão Depurar Projeto (image:debug-project-btn.png[]) na barra de ferramentas principal do IDE. O GlassFish Server será iniciado (ou reiniciado, se já estiver sendo executado) e abrirá um soquete no número da porta de depuração. A página de boas-vindas da aplicação será aberta no browser.
...
image:breakpoint-suspended.png[title="O depurador é suspenso em pontos de interrupção no editor"]
9. Posicione o cursor na chamada para `getSession()` e pressione Ctrl-Espaço para chamar a documentação do Javadoc.
image:javadoc-getsession.png[title="Pressione Ctrl-Espaço para chamar a documentação do Javadoc"]
...
image:javadoc-additem.png[title="Passe o mouse sobre métodos no Navegador para exibir documentação do Javadoc"]
...
image:session-variable.png[title="Passe o mouse sobre variáveis e expressões para determinar seus valores atuais"]
11. Clique no botão Fazer Step Over (image:step-over-btn.png[]) na barra de ferramentas do depurador, localizada acima do editor. A linha será executada e o depurador passará para a próxima linha do arquivo.
...
link:session-variable-set.png[image:session-variable-set.png[title="Clique para aumentar"]]

No NetBeans 6.9, você pode clicar no ponteiro cinza (image:grey-pointer.png[]) no popup para expandir uma lista de valores de variáveis contidos no elemento destacado.

13. Clique o botão Fazer Step Over (image:step-over-btn.png[]) (F8; fn-F8 no Mac) para chegar à instrução `if` (linha 154). Como você acabou de clicar no botão "adicionar ao carrinho" no browser, deve sabe que a expressão `userPath.equals("/addToCart)` deverá ser avaliada como `true`.
...
image:expression.png[title="Destaque as expressões para determinar seus valores atuais"]
...
image:cart-null.png[title="O objeto carrinho não existe, até que o usuário adicione item ao carrinho de compras"]
16. Pressione F8 (fn-F8 no Mac) para passar à próxima linha (linha 160). Depois, na linha 160, onde o objeto `ShoppingCart` é criado, clique no botão Fazer Step Into (image:step-into-btn.png[]). O depurador entra no método que está sendo chamado. Neste caso, você será levado diretamente ao construtor do `ShoppingCart`.
image:cart-constructor.png[title="Fazer step into nos métodos para seguir a execução de runtime para outras classes"]
17. Pressione Ctrl-Tab para voltar ao `ControllerServlet`. Observe que o IDE fornece um Emblema (image:call-stack-badge.png[]) de Pilha de Chamada na linha 160, indicando que o depurador está suspenso temporariamente em algum lugar em um método mais no início da pilha de chamada.
...
link:variables-win-session.png[image:variables-win-session.png[title="Clique para aumentar"]]
...
link:variables-win-session-cart.png[image:variables-win-session-cart.png[title="Clique para aumentar"]]
...
image:create-watch.png[title="Crie controles em variáveis, à medida que percorre o código em uma sessão de depuração"]
...
image:new-watch-dialog.png[title="Clique com o botão direito do mouse nas variáveis e expressões no editor e escolha Novo Watch"]

Um novo watch será criado na variável `session` e em todas as variáveis que ela contiver. O watch é visível na janela Watches (Janela >Depuração >Watches) ou, se você alternar o botão Watches (image:watch-btn.png[]) na margem esquerda da janela Variáveis, ele será exibido na linha superior da janela Variáveis.
...
22. Clique na botão Fazer Step Into para (image:step-into-btn.png[]) fazer step into no método `addItem` do `ShoppingCart`.
...
24. Examine a variável `session` na qual você criou um watch (link:#step21[etapa 21] acima). A instrução `items.add(scItem)` na linha 51 adicionou o novo `ShoppingCartItem` à lista de `items` no `ShoppingCart`. Isso fica evidente ao entrar no terceiro atributo (ex., a variável `cart`) contida na sessão.
link:variables-window-add-item.png[image:variables-window-add-item.png[title="Clique para aumentar"]]
...
image:breakpoint-jsp.png[title="Você pode suspender o depurador nas páginas JSP"]
26. Clique no botão Continuar ( image:continue-btn.png[] ) na barra de ferramentas do depurador. O depurador continua agindo até que a execução seja concluída ou até atingir outro ponto de interrupção. Nesse caso, o depurador é suspenso na linha 86 no fragmento JSP do cabeçalho.
...
link:variables-window-number-of-items.png[image:variables-window-number-of-items.png[title="Clique para aumentar"]]
...
29. Clique no botão Finalizar Sessão (image:finish-session-btn.png[]). O runtime finaliza a execução e a sessão de depuração é encerrada. O browser exibe uma página de categoria totalmente renderizada e você poderá ver que o widget carrinho de compras no cabeçalho da página contém um item.
...
* (image:step-out.png[]) *Fazer Step Out:* você sai da chamada do método atual. Execute e remova o método de chamada superior da pilha de chamadas.
* (image:run-to-cursor.png[]) *Executar até o Cursor:* executa até a linha na qual o cursor está posicionado.
* (image:apply-code-changes.png[]) *Aplicar Alterações de Código:* depois de editar um arquivo, você pode pressionar este botão para que o arquivo seja recompilado e as alterações levadas em conta na sessão de depuração.
* (image:step-over-expression.png[]) *Fazer Step Over da Expressão:* permite que você exiba os parâmetros de entrada e os valores de saída resultantes de cada chamada de método dentro de uma expressão. Você pode inspecionar os valores de saída do método anterior e os parâmetros de entrada para o próximo método na janela Variáveis Locais. Quando não há mais nenhuma chamada de método, Fazer Step Over da Expressão se comporta como o comando Fazer Step Over (image:step-over-btn.png[]).
...
* link:#http-monitor[Analisando a Comunicação Cliente-Servidor com o Monitor HTTP]
* link:#url-rewrite[Mantendo Sessões com a Reescrita de URL]
...
image:servers-win-http-monitor.png[title="Selecione a opção Ativar Monitor HTTP para ativar o Monitor HTTP"]
2. Se o servidor já estiver sendo executado, será necessário reiniciá-lo. No entanto, como pretende usar o depurador e executar o depurador reinicia o servidor para se comunicar em uma porta diferente, simplesmente clique no botão Depurar Projeto (image:debug-project-btn.png[]) no na barra de ferramentas principal do IDE. Quando o servidor é reiniciado, uma sessão de depuração é iniciada e a página de boas-vindas da aplicação é aberta no browser. O monitor HTTP é exibido na parte inferior do IDE.
image:http-monitor.png[title="O Monitor HTTP é exibido por default na região inferior do IDE"]
...
image:session-tab.png[title="Detalhes da sessão são exibidos na guia Sessão no Monitor HTTP"]
...
image:get-session.png[title="O método getSession existe no servidor compilado da página JSP"]
...
7. No browser, selecione uma categoria e, em seguida, adicione um item ao seu carrinho. Volte para o IDE. Observe que o depurador é suspenso no ponto de interrupção no `ControllerServlet` que foi definido anteriormente (linha 150). Todos os pontos de interrupção são lembrados entre as sessões. Para remover o ponto de interrupção, você poderia clicar no emblema do ponto de interrupção (image:breakpoint-badge.png[]) na margem esquerda do editor. No entanto, como há vários pontos de interrupção já definidos no projeto, abra a janela Pontos de interrupção do depurador (Janela > Depuração > Pontos de Interrupção).
image:breakpoints-window.png[title="Exibir todos os pontos de interrupção em seu projeto na janela Pontos de interrupção"]
...
9. Clique no botão Continuar (image:continue-btn.png[]). A execução da solicitação é finalizada e a página da categoria será exibida no browser com um item adicionado ao carrinho.
...
Clique no botão Classificação Ascendente (image:ascending-sort-btn.png[]) para que os registros mais recentes sejam listados na parte superior.
...
link:http-monitor-add-to-cart.png[image:http-monitor-add-to-cart.png[title="Clique para aumentar"]]
...
image:cookies-tab.png[title="Cookies são exibidos na guia Cookies no Monitor HTTP"]
...
image:headers-tab.png[title="Cookies são exibidos na guia Cookies no Monitor HTTP"]
...
image:session-tab-add-to-cart.png[title="Atributos de sessão são exibidos na guia Sessão no Monitor HTTP"]
...
15. Alterne para o IDE e note que o depurador está suspenso no ponto de interrupção definido no `ControllerServlet`. Clique no botão Fazer Step Over (image:step-over-btn.png[]) para que a variável `session` seja atribuída ao objeto da sessão.
...
18. Clique no botão Finalizar Sessão (image:finish-session-btn.png[]) para encerrar a sessão de depuração.
...
image:firefox.png[title="Desative os cookies temporariamente no browser."]
...
image:compromised.png[title="A funcionalidade da aplicação fica comprometida quando o cliente não aceita cookies"]
...
* link:#time-interval[Definindo Intervalos de Tempo da Sessão]
* link:#programmatically[Tratando de Forma Programática Time-outs de Sessão]
...
image:go-to-file.png[title="A caixa de diálogo Ir para Arquivo permite navegação rápida para os arquivos do projeto"]
...
image:session-timeout.png[title="Especifique o time-out de sessão para a aplicação na guia Geral do web.xml"]
...
image:cart-page-session-intact.png[title="A página do carrinho depende de um objeto da sessão para exibir itens no carrinho de compras"]
...
image:glassfish-error-report.png[title="NullPointerException ocorre quando uma solicitação de sessão expirada é recebida"]
...
link:gf-server-output.png[image:gf-server-output.png[title="Clique para aumentar"]]
...
image:nullpointer-exception.png[title="O emblema do erro e a dica de ferramenta indicam a localização e a causa do problema"]
...
7. Clique no botão Novo Arquivo (image:new-file-btn.png[]) na barra de ferramentas do IDE. (Como alternativa, pressione Ctrl-N; ⌘-N no Mac.)
...
14. Abra a janela Pontos de Interrupção (Alt-Shift-5; Ctrl-Shift-5 no Mac) e verifique se não há nenhum ponto de interrupção existente definido. Para deletar um ponto de interrupção, clique com o botão direito do mouse no ponto de interrupção e selecione Deletar. (Se tiver concluído o exercício acima, link:#http-monitor[Examinando a Comunicação Cliente-Servidor com o Monitor de HTTP], poderá ter um ponto de verificação pendente definido no `ControllerServlet`.)
15. Execute o depurador. Clique no botão Depurar Projeto (image:debug-project-btn.png[]) na barra de ferramentas principal do IDE.
...
image:filter-breakpoint.png[title="Definir um ponto de interrupção no método getSession"]
...
19. Clique no botão Fazer Step Over (image:step-over-btn.png[]) e, em seguida, passe o ponteiro do mouse sobre a variável `session`. Contanto que não tenha se passado um minuto desde que a solicitação anterior foi enviada, você verá que a variável foi designada a `StandardSessionFacade`. Isso representa o objeto de sessão para a solicitação.
image:session-exists.png[title="Passe o mouse sobre as variáveis para determinar seus valores atuais"]
...
image:session-null.png[title="Realce as expressões e passe o mouse sobre elas para determinar seu valor atual"]
...
25. Clique no botão Finalizar Sessão (image:finish-session-btn.png[]) para encerrar a sessão de depuração.
