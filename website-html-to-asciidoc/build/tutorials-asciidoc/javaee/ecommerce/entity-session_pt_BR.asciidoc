...
= entity-session_pt_BR
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  entity-session_pt_BR
:description: Apache NetBeans  entity-session_pt_BR
:toc: left
:toc-title:
...
* link:#whatEJB[O que são as Tecnologias EJB e JPA?]
* link:#whatSession[O que são Beans de Sessão?]
* link:#specification[Sobre Especificações e Implementações]
* link:#addEntity[Adicionando Classes de Entidade]
* link:#addSession[Adicionando Beans de Sessão]
* link:#access[Acessando Dados com EJBs]
...
image:mvc-diagram.png[title="Diagrama MVC da aplicação AffableBean"]
...
image:java-ee-container.png[title="Como um contêiner Java EE, o GlassFish v3 contém contêineres Web e EJB e o EclipseLink, o provedor de persistência"]
...
image:entity-from-database.png[title="Escolha uma origem de dados disponível para ter o IDE lido em tabelas de banco de dados"]
...
image:entity-classes-68.png[title="Netbeans 6.8: assistente Classes de Entidade do Banco de Dados, Etapa 3: Classes de Entidade"]
...
image:create-pu.png[title="Use a caixa de diálogo Criar Unidade de Persistência para gerar um arquivo persistence.xml"]
...
5. Prossiga para a link:#step7[etapa 7] abaixo.

===== NetBeans IDE 6.9

image:entity-classes-69.png[title="Netbeans 6.9: assistente Classes de Entidade do Banco de Dados, Etapa 3: Classes de Entidade"]
1. Digite *entity* (entidade) no campo Pacote. O assistente criará um novo pacote para as classes de entidade depois da conclusão.
...
image:entity-classes.png[title="Exibir novas classes de entidade na janela Projetos"]
...
image:embedded-id.png[title="Pressione Ctrl-Espaço para chamar a documentação da API"]
...
image:persistence-unit.png[title="View do Design da unidade de persistência AffableBeanPU"]
...
image:session-beans-for-entity-classes.png[title="Selecione Beans de Sessão para Classes de Entidade para gerar uma fachada da sessão para seu modelo de persistência"]
3. Clique em Próximo.
...
image:generated-session-beans.png[title="Especifique o local dos novos beans de sessão e se deseja criar interfaces"]
...
|image:projects-window-session-beans.png[title="Examinar novos beans de sessão na janela Projetos"] |image:projects-window-session-beans-69.png[title="Examinar novos beans de sessão na janela Projetos"] 
|===
...
* link:#index[página de índice]
* link:#category[página de categoria]
...
image:method-signature.png[title="Coloque o mouse sobre o método e mantenha pressionada a tecla Ctrl para exibir sua assinatura"]
...
image:override.png[title="Utilize a dica do IDE para adicionar a anotação @Overrides para o método"]
...
image:commented-out.png[title="Pressione Ctrl-/ para comentar um snippet de código no editor"]
...
----
11. Execute o projeto. Clique no botão Executar Projeto (image:run-project-btn.png[]). A página de índice do projeto será aberta no browser e você verá que todos os nomes e imagens das quatro categorias serão exibidos.
image:index-page.png[title="Verifique se a página de índice pode recuperar detalhes da categoria"]
...
* link:#categoryData[dados da categoria]
* link:#selectedCategoryData[categoria selecionada]
* link:#productData[dados do produto para a categoria selecionada]
...
image:comment-out-sql.png[title="Comente instruções JSTL <sql> na página da categoria"]
...
<c:forEach var="category" items="*${categories}*">
----
3. Execute o projeto para examinar o estado atual da página da categoria. Clique no botão Executar Projeto (image:run-project-btn.png[]). Quando a página de índice do projeto abrir no browser, clique em qualquer uma das quatro categorias. Os botões de categoria na coluna esquerda serão exibidos e funcionarão como esperado.
image:category-page-left-column.png[title="Os botões de categoria na coluna esquerdo serão exibidos e funcionarão conforme esperado"]
...
4. Clique no badge (image:editor-badge.png[]) na margem esquerda para usar a dica do editor para declarar `selectedCategory` como uma variável local dentro do método `doGet`.
image:local-variable.png[title="Usar dicas Editor para declarar variáveis locais"]
...
image:category-page-selected-category.png[title="O nome da categoria selecionada será exibido na página da categoria"]
...
image:local-variable2.png[title="Usar dicas Editor para declarar variáveis locais"]
...
image:category-page-product-table.png[title="A tabela Produto exibe produtos de uma determinada categoria"]
...
1. link:#1[^] Adaptado de link:http://www.manning.com/panda/[EJB 3 em ação] Capítulo 1, seção 1.1.2: EJB como framework.
2. link:#2[^] Há muitos outros serviços fornecidos pelo EJB. Para obter uma lista mais abrangente, consulte link:http://www.manning.com/panda/[EJB 3 em ação], Capítulo 1, seção 1.3.3: Ganhando funcionalidade com serviços EJB.
