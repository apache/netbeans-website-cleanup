...
:keywords: Apache NetBeans  cdi-intro_pt_BR
:description: Apache NetBeans  cdi-intro_pt_BR
:toc: left
:toc-title:
...
* link:#creating[Criação de um Projeto Java Web com o Suporte de CDI]
* link:#named[Acessando Beans a partir da Linguagem de Expressão do JSF]
* link:#upgrading[Atualizando para um EJB]
...
1. Clique no botão Novo Projeto (image:new-project-btn.png[]) na barra de ferramentas principal do IDE (Ctrl-Shift-N; ⌘-Shift-N no Mac).
...
image:new-web-application1.png[title="A opção CDI, quando selecionada, gera um arquivo beans.xml para o projeto"]
6. Clique em Próximo.
...
image:projects-window1.png[title="Novo projeto contém arquivo beans.xml do CDI e a biblioteca do GlassFish inclui o arquivo Weld JAR"]
...
image:new-java-class.png[title="Criar novas classes Java usando o assistente de Classe Java"]
...
image:facelets-el-completion.png[title="Criar novas classes Java usando o assistente de Classe Java"]
6. Clique no botão Executar Projeto (image:run-project-btn.png[]) na barra de ferramentas principal do IDE. O projeto será compilado e implantado no GlassFish e a página de boas-vindas da aplicação (`index.xhtml`) será aberta no browser. Você poderá ver a mensagem "Olá Mundo!" do `MessageServerBean` exibida na página.
image:browser-output1.png[title="A página de boas-vindas da aplicação exibe detalhes do MessageServerBean"]
...
image:browser-output-ejb1.png[title="Usar a anotação @stateless transforma o MessageServerBean em um EJB"]
