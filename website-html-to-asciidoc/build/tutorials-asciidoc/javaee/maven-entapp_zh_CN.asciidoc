...
:keywords: Apache NetBeans  maven-entapp_zh_CN
:description: Apache NetBeans  maven-entapp_zh_CN
:toc: left
:toc-title:
...
* link:#Exercise_1[创建 Maven 企业应用程序项目]
* link:#Exercise_2[对 EJB 模块进行编码]
* link:#Exercise_2a[创建持久性单元]
* link:#Exercise_2b[创建实体类]
* link:#Exercise_2c[创建消息驱动 Bean]
* link:#Exercise_2d[创建会话 Facade]
* link:#Exercise_3[对 Web 模块进行编码]
* link:#Exercise_3a[创建 ListNews Servlet]
* link:#Exercise_3b[创建 PostMessage Servlet]
* link:#Exercise_4[使用 Maven 构建应用程序]
* link:#Exercise_4a[处理项目依赖关系]
* link:#Exercise_4b[使用依赖关系构建]
* link:#Exercise_5[部署并运行应用程序]
* link:#Exercise_5a[从 IDE 中部署]
* link:#Exercise_5b[从 GlassFish Server 管理控制台中部署]
* link:#Exercise_7[下载解决方案项目]
...
* 如果您使用的是 NetBeans IDE 7.1 或更早版本，您将需要选择 "Window"（窗口）> "Other"（其他）> "Maven Repository Browser"（Maven 资源库浏览器），并在 "Maven Repository Browser"（Maven 资源库浏览器）窗口顶部单击 "Update Indexes"（更新索引）(image:maven-refreshrepo.png[title=""Update Indexes"（更新索引）按钮"])。
...
image:maven-newproject1.png[title="新建项目向导中的 "Maven Enterprise Application"（Maven 企业应用程序）项目类型"]
3. 键入 *MavenEnterpriseApp* 作为项目名称并设置项目位置。
4. （可选）修改工件详细信息。单击 "Next"（下一步）。
image:maven-newproject2.png[title="新建项目向导中的 "Maven Project"（Maven 项目）详细信息"]
...
image:maven-projectswindow2.png[title="显示已生成项目的 "Projects"（项目）窗口"]
...
image:priming-build.png[title="使用状态栏中的 Maven 图标可以运行初期构建"]
...
image:maven-persistenceunit.png[title="新建持久性单元向导"]
...
image:maven-messagedestination.png[title=""Add Message Destination"（添加消息目标）对话框"]
7. 确认项目目标正确无误。单击 "Next"（下一步）。
image:maven-newmdb.png[title="新建消息驱动 Bean 向导"]
...
image:maven-sessionwizard.png[title="实体类的会话 Bean 向导"]
...
image:maven-callbean.png[title=""Call Enterprise Bean"（调用企业 Bean）对话框"]
...
image:maven-searchdepend1.png[title=""Fix All Imports"（修复所有导入）对话框"]
...
image:maven-searchdependencies.png[title="编辑器中用于在资源库中搜索依赖关系的提示"]

使用 "Search in Maven Repositories"（在 Maven 资源库中搜索）对话框，可以找到包含 `ConnectionFactory` 的 `javaee-api-6.0` 工件。

image:maven-searchdepend2.png[title=""Search in Maven Repositories"（在 Maven 资源库中搜索）对话框"]
...
8. 修复导入并保存更改。
image:maven-jms-imports.png[title=""Fix All Imports"（修复所有导入）对话框"]
...
image:maven-earpom.png[title="EAR 项目的 pom.xml"]
...
image:maven-webpom.png[title="Web 应用程序项目的 pom.xml"]
...
image:maven-webpomgraph.png[title="依赖关系图"]
...
image:maven-addscope-javaee.png[title="POM 中作用域元素的代码完成"]
...
image:maven-reactor1.png[title="显示反应器构建顺序的 "Output"（输出）窗口"]

还会在 "Output"（输出）窗口中显示构建结果。

image:maven-reactor2.png[title="显示反应器构建状态的 "Output"（输出）窗口"]

在构建 EAR 项目后，您可以在 "Files"（文件）窗口中的 EAR 项目节点下方的 `target` 目录中看到最终的 EAR 档案。

image:maven-earfileswindow1.png[title="显示 EAR 档案的 "Files"（文件）窗口"]
...
image:maven-browser1.png[title="在浏览器中打开的 ListNews 页"]
3. 单击 "Add New Message"（添加新消息）。
4. 在 PostMessage Servlet 的窗体中键入消息。单击 "Submit Query"（提交查询）。
image:maven-browser2.png[title="在浏览器中打开的 PostMessage 页"]
