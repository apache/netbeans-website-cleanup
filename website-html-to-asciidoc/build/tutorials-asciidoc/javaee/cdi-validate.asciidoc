...
:keywords: Apache NetBeans  cdi-validate
:description: Apache NetBeans  cdi-validate
:toc: left
:toc-title:
...
* link:#alternative[Handling Multiple Deployments]
* link:#lifecycle[Applying Lifecycle Annotations to Managed Beans]
...
1. Begin by extracting the sample start project from the `cdiDemo2.zip` file (See the link:#requiredSoftware[table listing required resources] above.) Open the project in the IDE by choosing File > Open Project (Ctrl-Shift-O; ⌘-Shift-O on Mac), then selecting the project from its location on your computer.
...
12. Click the Run Project ( image:run-project-btn.png[] ) button in the IDE's main toolbar. The project is compiled and deployed to GlassFish, and the application's welcome page (`process.xhtml`) opens in the browser.
...
16. Click the Run Project ( image:run-project-btn.png[] ) button to run the project. Note that the project now fails to deploy.
...
19. Use the IDE's Go to File dialog to quickly open the `beans.xml` file. Choose Navigate > Go to File from the IDE's main menu (Alt-Shift-O; Ctrl-Shift-O on Mac), then type '`beans`'. Click OK. image:go-to-file.png[title="Use the Go to File dialog to quickly locate a project file"]
...
21. Click the Run Project ( image:run-project-btn.png[] ) button to run the project (Alternatively, press F6; fn-F6 on Mac). In the browser, click the '`Execute`' button that displays on the page. Switch back to the IDE and examine the GlassFish server log displayed in the Output window (Ctrl-4; ⌘-4 on Mac).
[source,java]
----
...
9. Click the Run Project ( image:run-project-btn.png[] ) button to run the project (Alternatively, press F6; fn-F6 on Mac). In the browser, click the '`Execute`' button that displays on the page. Switch back to the IDE and examine the GlassFish server log displayed in the Output window (Ctrl-4; ⌘-4 on Mac).
[source,java]
----
