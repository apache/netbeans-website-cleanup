...
= profiler-javaee_zh_CN
:jbake-type: page
:jbake-tags: old-site, needs-review
:jbake-status: published
:keywords: Apache NetBeans  profiler-javaee_zh_CN
:description: Apache NetBeans  profiler-javaee_zh_CN
:toc: left
:toc-title:
...
* link:#aboutear[关于对企业应用程序进行性能分析]
* link:#profear[对企业应用程序项目进行性能分析]
* link:#profejb[对 EJB 和 Web 模块子项目进行性能分析]
* link:#create[创建样例项目]
* link:#monitor[监视应用程序]
* link:#cpu[分析应用程序的性能]
* link:#instfilter[使用分析过滤器]
* link:#sample[对应用程序进行采样]
* link:#cpuentire[分析所有方法]
* link:#profroot[使用性能分析根方法]
* link:#memory[分析内存使用情况]
...
image:monitor-memory-telemetry.png[title="显示垃圾回收信息的内存（垃圾回收）图"]
...
image:sample-task.png[title="在 &quot;Select Profiling Task&quot;（选择性能分析任务）对话框 &quot;CPU&quot; 窗格中选中了 &quot;Sample Application&quot;（样例应用程序）模式"]
...
image:sample-task-results.png[title="&quot;Sample Application&quot;（样例应用程序）模式的结果"]
...
5. 从 "Filter"（过滤器）下拉列表中选择 *Profile only project classes*（只分析项目类）。单击 "Run"（运行）。
image:advanced-cpu-task.png[title="在 &quot;Select Profiling Task&quot;（选择性能分析任务）对话框 &quot;CPU&quot; 窗格中选中了 &quot;Profile Application&quot;（分析应用程序）模式"]
...
image:cpu-pane1-results.png[title="&quot;Profile Application&quot;（分析应用程序）模式的结果"]
...
image:cpu-pane1-results1.png[title="&quot;Profile Application&quot;（分析应用程序）模式的结果"]
...
image:drill-down.png[title="应用程序中的 Servlet 所花费的相对时间的详细视图图"]
...
image:profileroot-sayhello-project.png[title="应用程序中的 Servlet 所花费的相对时间的详细视图图"]

如果选择 "Profile all classes"（分析所有类）过滤器，则 IDE 将分析 `sayHello` 调用的所有方法，包括所有 Java 核心类。

image:profileroot-sayhello-all.png[title="应用程序中的 Servlet 所花费的相对时间的详细视图图"]
...
image:edit-profiling-roots.png[title="&quot;Edit Profiling Roots&quot;（编辑性能分析根目录）对话框中的 &quot;Web Application View&quot;（Web 应用程序视图）"]

此外，也可以选择 "Package View"（包视图），以查看应用程序中的所有源包，然后在 `servlet.stateless` 包中选择 `Servlet2Stateless` 类中的 `service` 方法。

image:edit-profiling-roots-pkg.png[title="&quot;Edit Profiling Roots&quot;（编辑性能分析根目录）对话框中的 &quot;Package View&quot;（包视图）"]
...
image:edit-profilingroots-pkg-adv.png[title="&quot;Edit Profiling Roots (Advanced)&quot;（编辑性能分析根目录 (高级)）对话框"]
...
image:advanced-cpu-task-custom.png[title="对定制性能分析根目录选择了 &quot;Profile Application&quot;（性能分析应用程序）模式"]

在单击 "Run"（运行）后，IDE 将构建和部署应用程序，并在 Web 浏览器中打开 Servlet 页。

IDE 将在 "Live Profiling Results"（实时性能分析结果）标签中显示性能分析结果。

image:cpu-pane2-results.png[title="使用性能分析根目录分析方法的结果"]
...
image:cpu-pane2-results1.png[title="在调用 sayHello 方法后使用性能分析根目录分析方法的结果"]
...
image:cpu-pane2-results2.png[title="在调用 sayHello 方法后使用性能分析根目录分析方法的结果"]
...
image:custom-filter.png[title="首次对项目进行性能分析时所显示的对话框"]
...
image:set-quick-filter.png[title="首次对项目进行性能分析时所显示的对话框"]
...
image:profile-memory-advanced.png[title="显示内存任务和选项的 &quot;Select Profiling Task&quot;（选择性能分析任务）对话框"]
...
image:profile-memory-results.png[title="显示内存使用情况性能分析结果的 &quot;Profiling Results&quot;（性能分析结果）标签"]
